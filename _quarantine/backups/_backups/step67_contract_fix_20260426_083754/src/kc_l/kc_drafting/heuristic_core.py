...
def _definition_signal(row: Mapping[str, Any], text_norm: str) -> bool:
    role_hint = dict(row.get("role_hint") or {})
...
def assess_overlay_candidate(row: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    supportive_context = bool(row.get("strong_structured_candidate")) or bool(row.get("strong_same_topic"))
    ...
    context_candidate = bool(explanatory_candidate)
...
def _definition_context_tokens(row: Mapping[str, Any]) -> set[str]:
    # Active seedless drafting uses typed hierarchy context rather than any
    # injected seed definition text.
...
def _definition_alignment_features(row: Mapping[str, Any], text: str) -> tuple[int, int, bool]:
    ...
    seed_overlap = len(text_tokens & _definition_context_tokens(row))
    has_name_anchor = _has_name_anchor(text, row)
    return title_overlap, seed_overlap, has_name_anchor
...
def _definition_surface_passes_gate(
    ...
    seed_overlap = len(text_tokens & _definition_context_tokens(row))
    has_name_anchor = _has_name_anchor(text, row)
    has_target_alignment = bool(title_overlap > 0 or seed_overlap > 0 or has_name_anchor)
...
def _definition_surface_choice_score(
    ...
    score += 2.0 * float(min(seed_overlap, 4))
...
def _definition_packet_role(
    *,
    ...
    if bool(has_name_anchor or title_overlap > 0 or seed_overlap > 0):
...
def build_definition_evidence_packet(
    ...
    *,
    ...
    for candidate_id, row in rows_by_id.items():
        ...
        role_score += 2.0 * float(min(seed_overlap, 3))
