...
from kc_l.kc_drafting.contracts import (
    ...
    SURVIVAL_FLOOR_STATUS_NAME_ONLY,
    SURVIVAL_FLOOR_STATUS_SEED,
    TRUST_STATE_FALLBACK_SEED,
    TRUST_STATE_FALLBACK_SEED_WITH_RISKS,
...
SEED_DEFINITION_FLOOR_ACTIVE_FLAG = "seed_definition_floor_active"
...
LOW_TRUST_SURVIVOR_FLAG = "low_trust_survivor"
REVIEW_NEEDS_ATTENTION_FLAG = "review_needs_attention"
LOW_TRUST_REVIEW_FLAG = "low_trust_review"
...
def _definition_binding_candidate_analysis(
    ...
    *,
    ...
    if not candidate.has_name_anchor and candidate.title_overlap <= 0 and candidate.seed_overlap <= 0:
    ...
    binding_score += 1.0 * float(min(candidate.seed_overlap, 3))
...
def _run_definition_rescue_phase(
    *,
    ...
    preservation_short_circuits = bool(preserved_definition_candidate) and preserved_tier >= 4
...
def _salvage_surface_alignment_signals(
    ...
    *,
    ...
    strong_alignment = bool(
        named_alignment
        or int(target_profile.get("seed_overlap") or 0) >= 3
        or int(candidate.title_overlap) >= 2
        or int(candidate.seed_overlap) >= 3
    )
...
def _local_row_source_faithful_normalized_candidate(
    *,
    ...
    comparison_tokens = _content_token_set(draft_text) | _content_token_set(seed_definition)
...
def _seed_definition_floor(exemplar: Mapping[str, Any], *, definition_grounded: bool) -> Dict[str, Any]:
    seed_definition = _normalize_ws(exemplar.get("seed_definition") or "")
    canonical_name = _normalize_ws(exemplar.get("canonical_name") or "")
    text = seed_definition or canonical_name
    status = SURVIVAL_FLOOR_STATUS_SEED if seed_definition else SURVIVAL_FLOOR_STATUS_NAME_ONLY
    source_field = "hierarchy.seed_definition" if seed_definition else "hierarchy.canonical_name"
    return {
        "status": status,
        "text": text,
        "canonical_name": canonical_name,
        "source_field": source_field,
        "used_as_definition_fallback": not definition_grounded,
    }
...
def _authoritative_definition_status(
    *,
    ...
    return AUTHORITATIVE_DEFINITION_STATUS_SEED_FLOOR_FALLBACK
...
def _semantic_risk_flags(
    *,
    ...
    if not definition_grounded:
        flags = unique_preserve_order([*flags, SEED_DEFINITION_FLOOR_ACTIVE_FLAG, DEFINITION_ENRICHMENT_MISSING_FLAG, LOW_TRUST_SURVIVOR_FLAG])
...
def _trust_state(
    *,
    ...
    if definition_grounded and scope_grounded and context_status == CONTEXT_LAYER_STATUS_GROUNDED and not any(
        ...
        label = TRUST_STATE_FALLBACK_SEED_WITH_RISKS
    else:
        label = TRUST_STATE_FALLBACK_SEED
    return {
        "label": label,
        "definition_grounded": definition_grounded,
        "scope_grounded": scope_grounded,
        "context_status": context_status,
        "low_trust": label in {TRUST_STATE_FALLBACK_SEED, TRUST_STATE_FALLBACK_SEED_WITH_RISKS},
    }
...
def _review_readiness(
    *,
    ...
    elif trust_label in {TRUST_STATE_FALLBACK_SEED, TRUST_STATE_FALLBACK_SEED_WITH_RISKS}:
        label = REVIEW_READINESS_LOW_TRUST
    else:
    ...
    readiness_reasons = unique_preserve_order(
        [
            ...
            *[
                ...
                if str(item) in {
                    SEED_DEFINITION_FLOOR_ACTIVE_FLAG,
...
def build_kc_draft_bundles_llm(
    ...
    *,
    ...
    for kc_id in sorted(rows_by_kc.keys()):
        ...
        survival_floor = _seed_definition_floor(exemplar, definition_grounded=definition_grounded)
        ...
        if bool(survival_floor.get("used_as_definition_fallback")):
            seed_floor_active_count += 1
        if context_status == CONTEXT_LAYER_STATUS_FALLBACK:
    ...
    stats = {
        ...
        "seed_floor_active_count": seed_floor_active_count,
