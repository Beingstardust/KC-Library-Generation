...
def _choose_definition_payload(bundle: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
...
def _scope_payload(bundle: Mapping[str, Any]) -> dict[str, Any]:
...
def _build_evidence_spans(bundle: Mapping[str, Any], scope_support_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for item in bundle.get("evidence_bundle") or []:
        ...
        if not overlay_candidate_id or not doc_id or not block_id or not layer or not quote or not isinstance(page_index, int):
            continue
        provenance_flags = [
...
def _build_coverage_note(bundle: Mapping[str, Any], risk_flags: Sequence[str]) -> str:
    parts: list[str] = []
    scope_payload = _scope_payload(bundle)
    if _as_text(scope_payload.get("status")) != "grounded":
...
def _priority_features(
    ...
    *,
    ...
    draft_status = _as_text(bundle.get("draft_status"))
    support_summary = dict(bundle.get("support_summary") or {})
    scope_payload = _scope_payload(bundle)
    semantic_tier = 2
    if draft_status == "draft_ready_with_holds" or REVIEW_QUEUE_WEAK_COVERAGE_FLAG in risk_flags:
        semantic_tier = 1
    definition_status = "coherent_supported" if _as_text(support_summary.get("support_state")) == "strict_leaf_support" else "fragmentary_supported"
    ...
    return {
        ...
        "support_contract_downgraded": False,
        "contamination_category": "clean",
        "sibling_ambiguity": mixed_foreign_definition_support,
        "operational_support_present": _as_text(scope_payload.get("status")) == "grounded",
    }
...
def _build_notes_for_reviewer(bundle: Mapping[str, Any], risk_flags: Sequence[str]) -> str:
    notes: list[str] = []
    if _as_text(_scope_payload(bundle).get("status")) != "grounded":
...
def build_restarted_review_packet(
    *,
    ...
    definition_payload, definition_source = _choose_definition_payload(draft_bundle)
    definition_draft = _as_text(definition_payload.get("text"))
    held_salvage_notes: list[str] = []
    held_salvaged = False

    if draft_status not in {"draft_ready", "draft_ready_with_holds"}:
    ...
    if not kc_id or not canonical_name or not definition_draft:
    ...
    scope_payload = _scope_payload(draft_bundle)
    scope_status = _as_text(scope_payload.get("status")) or "abstained"
    scope_draft = _as_text(scope_payload.get("text")) if scope_status == "grounded" else ""
    scope_support_ids = set(_normalize_string_list(scope_payload.get("supporting_overlay_candidate_ids")))
    evidence_spans = _build_evidence_spans(draft_bundle, scope_support_ids)
    if not evidence_spans:
        raise ValueError("no_grounded_step6_7_evidence_bundle")

    risk_flags = _normalize_string_list(draft_bundle.get("contamination_flags"))
...
def build_summary(
    *,
    ...
    held_salvage_ready_ids = [
        packet["kc_candidate_id"]
        for packet in packets
        if HELD_REVIEW_SALVAGE_FLAG in _normalize_string_list(packet.get("risk_flags"))
    ]
    held_salvage_quarantine_ids = [
        _as_text(item.get("kc_candidate_id"))
        for item in quarantine
        if HELD_REVIEW_SALVAGE_FLAG in _normalize_string_list(item.get("risk_flags"))
    ]
    caution_ids = [
        packet["kc_candidate_id"]
        for packet in packets
    ...
    quarantine_draft_status_counts = Counter(item.get("draft_status") for item in quarantine)
    return {
        "schema_version": "1.0",
        ...
        "quarantined_kcs": [dict(item) for item in quarantine],
        "excluded_kcs": [
            {
        ...
        "held_salvage_ready_count": len(held_salvage_ready_ids),
        "held_salvage_ready_kcs": held_salvage_ready_ids,
        "held_salvage_quarantine_count": len(held_salvage_quarantine_ids),
        "held_salvage_quarantined_kcs": held_salvage_quarantine_ids,
        "recommendation_counts": dict(recommendation_counts),
        "priority_counts": dict(priority_counts),
        "draft_status_counts": dict(draft_status_counts),
        "quarantine_draft_status_counts": dict(quarantine_draft_status_counts),
        "risk_flag_counts": dict(risk_flag_counts),
        "field_source_map": {
            "definition_draft": "step6_7.kc_draft_bundles.definition_full_candidate or definition_short_candidate",
            "scope_draft": "step6_7.kc_draft_bundles.scope_candidate when grounded; blank when reviewer-editable gap is preserved",
            "field_provenance_map": "step6_7.kc_draft_bundles.field_provenance_map",
            "evidence_spans": "step6_7.kc_draft_bundles.evidence_bundle",
            "risk_flags": "step6_7 draft contamination flags plus explicit scope-gap metadata",
            "review_priority": "src/kc_l/kc/supervision.py computed from conservative features derived from the Step 6.7 draft bundle",
        },
        "grounding_rules": {
            "ready_only_rule": "Only Step 6.7 packets that survive the Step 6.8 quarantine boundary are emitted as ready review packets.",
            "quarantine_rule": "Packets with fragmentary reviewer-facing definition surfaces or mixed-foreign decisive definition support are quarantined out of the ready review lane.",
            "held_exclusion_rule": "Held Step 6.7 drafts that remain non-review-salvageable stay outside the ready packet set and are only reported as exclusions.",
            "held_salvage_rule": "Held Step 6.7 bundles may re-enter the ready review lane only when a persisted local-only LLM definition proposal is coherent enough for review and does not rely on sibling-mismatched support.",
            "scope_gap_rule": "Missing scope is preserved as an explicit reviewer-editable gap and never invented; by itself it does not exclude a coherent held-salvage definition from review.",
...
def build_preview_markdown(
    *,
    ...
    lines = [
        ...
        f"- Quarantined KCs: `{summary.get('quarantined_kcs', [])}`",
        f"- Excluded KCs: `{summary['excluded_kcs']}`",
    ...
    if summary.get("quarantined_kcs"):
        lines.extend(["## Quarantined KCs", ""])
        for item in summary["quarantined_kcs"]:
            lines.append(f"- `{item['kc_candidate_id']}` (`{item['draft_status']}`): {', '.join(item['reasons'])}")
    if summary["excluded_kcs"]:
        lines.extend(["## Excluded KCs", ""])
        for item in summary["excluded_kcs"]:
            lines.append(f"- `{item['kc_candidate_id']}` (`{item['draft_status']}`): {', '.join(item['reasons'])}")
    return "\n".join(lines).rstrip() + "\n"
...
def emit_restarted_review_packets_from_draft_bundles(
    *,
    ...
    exclusions: list[PacketExclusion] = []

    for row in draft_rows:
        draft_status = _as_text(row.get("draft_status"))
        ...
        if draft_status == "held":
            try:
                ...
                reasons = _normalize_string_list(row.get("hold_reasons")) or [EXCLUDED_HELD_REASON]
                error_reason = _as_text(exc)
                if error_reason and error_reason not in reasons:
        ...
        quarantine_reasons = _quarantine_reasons(packet)
            ...
            quarantined.append(
                {
                    ...
                    "definition_draft": packet.get("definition_draft"),
    ...
    return RestartedReviewPacketEmissionResult(
        ...
        packet_count=len(packets),
        excluded_candidate_count=len(exclusions) + len(quarantined),
    )
