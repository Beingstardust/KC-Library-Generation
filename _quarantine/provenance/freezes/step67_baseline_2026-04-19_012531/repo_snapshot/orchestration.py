...
def run_step6_75_pipeline(
    *,
    ...
    bundle_path = resolve_repo_path(step6_7_set_obj.get("artifacts", {}).get("kc_draft_bundles_jsonl"), base_dir=repo_root)
    overlay_path = resolve_repo_path(
...
def run_step6_8_pipeline(
    *,
    ...
    step6_6_set_obj = read_json(step6_6_set_manifest_path)
    step1_registry_path = resolve_repo_path(
        step6_6_set_obj.get("upstream", {}).get("kc_registry_jsonl")
        or "data/work/cache/current_step_artifacts/step1_kc_registry.current.jsonl",
        base_dir=repo_root,
    )
