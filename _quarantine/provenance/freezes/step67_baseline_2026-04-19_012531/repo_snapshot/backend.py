...
def ensure_llm_runtime_available(execution: Mapping[str, Any]) -> List[str]:
    generation_model_alias = as_text(execution.get("generation_model_alias"))
    ...
    provider = as_text(execution.get("provider") or "ollama").lower()
    if provider != "ollama":
        raise RuntimeError(f"Unsupported Step 6.7 LLM provider: {provider}")
...
class OllamaDraftingBackend:
    ...
    def _runtime(self) -> Step67ModelRuntime:
        return Step67ModelRuntime(
            ...
            model=as_text(self.execution.get("generation_model_alias")),
