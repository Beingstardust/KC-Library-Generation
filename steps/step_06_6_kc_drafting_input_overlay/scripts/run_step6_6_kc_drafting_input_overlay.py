...
def merge_missing_aliases_from_evidence_packs(
    registry_rows: List[Dict[str, Any]], evidence_packs_by_kc: Mapping[str, Mapping[str, Any]]
) -> None:
    """registry_rows read from kc_registry_path (the hierarchy-build scaffold file) never carry
    aliases - that field is only ever populated later, during step5x evidence-pack assembly, as
    enrichment derived from the actual retrieved evidence. Whenever kc_registry_path happens to
    exist, the branch above reads it directly and silently drops whatever richer aliases are
    sitting in evidence_packs_by_kc, with no signal that anything was lost (confirmed via a real
    run where this zeroed out aliases for all 159 KCs despite 150 of them having real aliases in
    their own evidence pack row). This backfills in place - only when a row's own aliases are
    empty and a same-kc_id evidence-pack row has real ones - so scaffold-provided rows still win
    when they legitimately do carry aliases, and no other field on the row is touched.
    """
    if not evidence_packs_by_kc:
        return
    for row in registry_rows:
        if row.get("aliases"):
            continue
        kc_id = str(row.get("kc_id") or row.get("knowledge_unit_id") or "")
        pack_row = evidence_packs_by_kc.get(kc_id)
        if not pack_row:
            continue
...
def main() -> int:
    ...
    # 2026-07-26: kc_registry_path no longer resolves through step_05_3's own upstream manifest
    # (step_05_3 dismantled - see drafting_input_overlay.py's SOURCE_STEP_KIND comment).
    # input_cfg.get("kc_registry_path") is a new optional override - the orchestrator now passes
    # THIS run's own real hierarchy_registry output (same resolve_hierarchy_manifest_from_
    # output_root() pattern already proven for step_05_3's own config) when available, since that
    # file carries richer/authoritative hierarchy_path data than reconstructing registry rows
    # from evidence-pack topic_path_labels alone (see the topic_pack_rows_missing_from_overlay_
    # hierarchy failure this masked before the override existed: 2 of 20 real topics dropped out
    # of the KC-level hierarchy paths entirely when reconstructed that way, confirmed real
    # incident this same investigation). overlay_manifest.get("normalized_registry") remains only
    # as the last-resort fallback when no override is supplied - itself confirmed STALE (a frozen
    # 2026-04-24 144-row registry vs a real run's 159) and never the live orchestrator's path.
    kc_registry_path = resolve_repo_path(
        input_cfg.get("kc_registry_path") or overlay_manifest.get("normalized_registry")
    )
    sentence_corpus_path = resolve_repo_path(step4_5_set_obj.get("artifacts", {}).get("sentence_corpus_jsonl"))
    ...
    # 2026-07-26: kc_registry_path.exists() is trusted first again now that the orchestrator
    # supplies a real, run-scoped override (see kc_registry_path's own comment above) - it
    # carries authoritative hierarchy_path/topic_path data that reconstructing registry rows
    # from evidence-pack topic_path_labels alone was dropping for a handful of topics (confirmed
    # real incident: 2 of 20 topics missing from the reconstructed hierarchy, failing step_06_7's
    # own topic_pack_rows_missing_from_overlay_hierarchy check). Falls back to the assembly-
    # authorized evidence-pack reconstruction only when no registry file is available at all -
    # still better than crashing, just less complete for topic-level hierarchy cross-referencing.
    ...
    merge_missing_aliases_from_evidence_packs(registry_rows, evidence_packs_by_kc)
    ...
    # step_05_3 dismantled - no review queue producer remains wired into this stage; kept as an
    # empty lookup (not removed as a build_overlay_records parameter) since it is purely
    # auxiliary annotation (step5_3_review_queue_aux on each record), never load-bearing.
    ...
    overlay_records, overlay_stats = build_overlay_records(
        ...
        source_set_id=str(
            (assembly_bridge_info.get("bridge_set_manifest") or {}).get("set_id") if assembly_bridge_info else ""
        ),
    ...
    set_manifest = {
        ...
        "upstream": {
            ...
            "kc_registry_jsonl": rel_path(kc_registry_path) if kc_registry_path.exists() else "",
