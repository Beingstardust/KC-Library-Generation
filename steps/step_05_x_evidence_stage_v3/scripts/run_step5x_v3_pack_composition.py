...
from kc_l.retrieval_gate.step5x_pack_sanitizer import finalize_step5x_pack_in_place
...
def main() -> int:
    ...
    # Fold-in of the ad-hoc finalize_best_step5x_pack_gapaware_v2.py logic (confirmed this
    # session to be the only genuinely non-canonical step still missing from this chain - see
    # ORCHESTRATOR_BUILD_STATE.md's "KC-track gate-chain quality investigation" entry): dedup
    # duplicate ordered items within a KC, remove explicit exercise/task-prompt items, flag
    # (not remove) noisy-but-usable items, and create a retrieval_gap_request for any KC left
    # with zero ordered evidence solely because of the exercise-prompt removal.
    sanitizer_stats = finalize_step5x_pack_in_place(
        kc_evidence_packs_jsonl=REPO_ROOT / result["kc_evidence_packs_jsonl"],
        retrieval_gap_requests_jsonl=REPO_ROOT / result["retrieval_gap_requests_jsonl"],
        evidence_pack_stats_json=REPO_ROOT / result["evidence_pack_stats_json"],
        source_scored_candidates_jsonl=str(input_info.get("scored_candidates_jsonl") or ""),
        source_candidate_bank_jsonl="",
        run_id=str(args.run_id or result["run_id"]),
    )
    result["step5x_pack_sanitizer"] = {
        "removed_exercise_count": sanitizer_stats["removed_exercise_count"],
        "dropped_duplicate_count": sanitizer_stats["dropped_duplicate_count"],
        "flagged_item_count": sanitizer_stats["flagged_item_count"],
        "newly_zero_kcs": sanitizer_stats["newly_zero_kcs"],
        "added_gap_count": sanitizer_stats["added_gap_count"],
    }
