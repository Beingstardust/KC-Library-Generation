...
def resolve_review_packet_manifest(review_packet_set_path: Path) -> Dict[str, Any]:
    """Read a Step 6.8 review-packet manifest, supporting both the legacy (set_id/artifacts)
    schema and the v2 (run_id/outputs) schema.

    v2 manifests declare invariants.legacy_step68_runner_not_used and have no artifacts/set_id
    keys, so detect on run_id+outputs rather than assuming. Mirrors
    run_step6_9_kc_review_audit_ingestion.py's own resolve_review_packet_manifest() exactly -
    duplicated here rather than imported, matching this codebase's established convention of
    each step script being self-contained (no step-to-step imports anywhere in this repo).
    """
...
def main() -> int:
    ...
    step6_8_manifest_fields = resolve_review_packet_manifest(step6_8_set_path)
    source_step6_8_set_id = step6_8_manifest_fields["set_id"]
    review_packet_jsonl_path = step6_8_manifest_fields["review_packet_jsonl_path"]
    review_packet_summary_path = step6_8_manifest_fields["review_packet_summary_path"]
    ...
    result = resolve_restarted_review_audits(
        ...
        source_review_packet_jsonl=review_packet_jsonl_path,
