...
_REPO_SRC = pathlib.Path(__file__).resolve().parents[4] / "src"
if str(_REPO_SRC) not in sys.path:
    sys.path.insert(0, str(_REPO_SRC))

from kc_l.retrieval_gate.branch_scoping import (  # noqa: E402
    CROSS_BRANCH_FLAG,
    check_cross_branch_mismatch,
    load_page_branch_map,
)
from kc_l.step67_postprocess.builder import strip_upstream_evidence_leakage  # noqa: E402

_PAGE_BRANCH_MAP_PATH = pathlib.Path(__file__).resolve().parents[4] / "data/processed/branch_scoping/page_branch_map.json"
...
def _rows_ancestor_topic_labels(rows: Sequence[Mapping[str, Any]]) -> Optional[List[str]]:
    ...
    for row in rows:
        ...
        if labels:
            return list(labels)
    for row in rows:
        path = row.get("source_hierarchy_path")
        if path and len(path) > 1:
            return list(path[:-1])
    return None


def _item_is_cross_branch_mismatch(
    item: Mapping[str, Any], target_topic_path_labels: List[str], kc_id: Optional[str]
) -> bool:
    if not kc_id:
        return False
    ...
    is_mismatch, _detail = check_cross_branch_mismatch(
        _get_page_branch_map(),
        doc_id=doc_id,
        page_index=page_index,
        target_topic_path_labels=target_topic_path_labels,
        own_kc_id=kc_id,
    )
    return is_mismatch
...
def _sanitize_packet_strings(obj: Any) -> Any:
    ...
    internal call site, guarantees complete coverage regardless of which code path populated a
    given field. Safe to apply blindly to every string (IDs, doc_ids, hierarchy labels, etc.)
    since the patterns being stripped (Equation/Figure/Table+number, bracketed citation
    numbers) do not incidentally occur in any of this pipeline's own naming conventions.
    """
    if isinstance(obj, str):
        return strip_upstream_evidence_leakage(obj)
    if isinstance(obj, list):
        return [_sanitize_packet_strings(x) for x in obj]
    if isinstance(obj, dict):
        return {k: _sanitize_packet_strings(v) for k, v in obj.items()}
    return obj
...
def evidence_text_from_overlay(row: Mapping[str, Any], max_chars: int) -> str:
    for key in ("quote_surface", "original_quote_surface", "source_block_text", "original_source_block_text"):
        ...
        if isinstance(value, str) and value.strip():
            ...
            # Sanitize before clipping so the full pattern is visible, not cut at the boundary.
            return clip_text(strip_upstream_evidence_leakage(value), max_chars)
    return ""
...
def compact_overlay_evidence(row: Mapping[str, Any], max_chars: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "source_block_text": clip_text(strip_upstream_evidence_leakage(row.get("source_block_text") or ""), max_chars),
        "role_hint": row.get("role_hint") if isinstance(row.get("role_hint"), dict) else {},
        "support_profile": {
...
def compact_topic_evidence(item: Mapping[str, Any], max_chars: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "source_block_text": clip_text(strip_upstream_evidence_leakage(item.get("source_block_text") or ""), max_chars),
...
# admitted that very item for carrying formal_relation_anchor - the two rules cancelled each
# other out. Keeping one set consulted by both sites makes that class of drift impossible.
PROMOTING_ANCHOR_ROLES = (
    "definition_anchor",
    "definitional_anchor",
    "metric_formula_anchor",
    "formal_relation_anchor",
    "objective_function_anchor",
)


def _review_needed_item_is_useful_for_synthesis(
    item: Mapping[str, Any],
    target_topic_path_labels: Optional[List[str]] = None,
    ...
    # Models of Randomness (Approach 1/2) regressions (real page-410 decision-tree-randomization
    # content, explicitly demoted upstream, admitted anyway via this exact short-circuit).
    # CROSS_BRANCH_FLAG is included too so items demoted by the branch-scoping check below (or by
    # the equivalent check further upstream in pack composition) are respected here as well.
    ...
    # risk check (no suspected_false_positive/contamination/cross-branch flag), rejected solely
    # for a binding requirement the KC's own name can never satisfy. For exactly this
    # structurally-doomed case, "no_target_binding" alone is not treated as a hard exclusion -
    # every other hard-exclusion reason (false positive, subject mismatch, cross-branch, etc.)
    # still applies unchanged, and normal-binding KCs are completely unaffected.
    if weak_binding:
    ...
    # neural-network text via its own alias "trade-off"). Root-caused since: both cases are
    # genuine homonym collisions from a SINGLE, GENERIC-WORD alias ("binary", "trade-off")
    # matching completely unrelated content that happens to contain that common word - the core
    # scorer now tags exactly this ("definition_subject_mismatch_single_token_match" - see
    # evidence_stage_v3_scored_candidates.py's _definition_subject_alignment, a plain word-count
    # check on the actual matched text, not a hardcoded list of "risky" words). Both confirmed
    # bad cases carry this tag; the genuinely good K-Means procedural content that motivated the
    # original relaxation attempt (matched via the multi-word "K-means algorithm" phrase) does
    # not. Exempting now requires BOTH conditions - the candidate doesn't itself claim a
    # definitional role, AND the underlying match was not a single generic token - stricter than
    # either condition alone, since false positives here are more costly than missed rescues.
    if "definition_subject_mismatch" in risks and "definition_subject_mismatch_single_token_match" not in risks:
        role_tokens = set(roles.split())
        for key in ("role", "preferred_support_role"):
            val = str(item.get(key) or "").lower()
            if val:
                role_tokens.add(val)
        if not (role_tokens & set(PROMOTING_ANCHOR_ROLES)):
            hard_exclusion_risks = hard_exclusion_risks - {"definition_subject_mismatch"}

    if any(r in hard_exclusion_risks for r in risks):
        return False

    # Independent cross-branch check (defense in depth): catches items that reach this filter
    # without ever having been risk-flagged upstream.
    if target_topic_path_labels and _item_is_cross_branch_mismatch(item, target_topic_path_labels, kc_id):
        return False

    if routing == "positive_role_candidate":
        return True

    if any(x in roles for x in PROMOTING_ANCHOR_ROLES):
        return True

    # Mechanism E fix, continued: routing_recommendation "drop_from_positive_roles" is itself
    # frequently set upstream BECAUSE of the same no-target-binding determination (confirmed:
    # every remaining KC_CLF_UND_002 candidate at this point carries only "no_target_binding" in
    # risks, nothing else). For a structurally weak-binding KC with no other hard-exclusion
    # reason surviving the checks above, treat the demotion as a consequence of the same
    # unsatisfiable binding requirement rather than an independent quality judgment.
    if weak_binding and routing == "drop_from_positive_roles":
        return True
...
def select_kc_synthesis_evidence_from_embedded_pack(
    ...
    *,
    ...
    target_topic_path_labels = _rows_ancestor_topic_labels(rows)
    kc_id = next((row.get("kc_id") for row in rows if row.get("kc_id")), None)
    binding = _target_binding_profile_from_rows(rows)
    weak_binding = len(binding.get("distinctive_tokens") or []) < 2

    review_items = _unique_pack_items(
        item
        for item in _first_nonempty_pack_lane(rows, "review_needed_evidence")
        if _review_needed_item_is_useful_for_synthesis(item, target_topic_path_labels, kc_id, weak_binding)
    )
...
def select_target_bound_overlay_fallback_evidence(
    ...
    *,
    ...
    rejected_cross_branch_mismatch = 0
    weak_binding_score_rescues = 0
    selected: List[Dict[str, Any]] = []
    selected_ids: set[str] = set()
    ...
    for row in sorted(rows, key=score_overlay_evidence, reverse=True):
        ...
        if not _overlay_row_has_target_binding(row, binding):
            if weak_binding and score_overlay_evidence(row) >= WEAK_BINDING_SCORE_FLOOR:
                weak_binding_score_rescues += 1
            else:
        ...
        # completely different branch's section of the corpus. Confirmed as the real mechanism
        # behind the "Handling Missing Values in NB" regression (decision-tree missing-value
        # content bound to a Naive Bayes KC via exactly this token-overlap path).
        if target_topic_path_labels and _item_is_cross_branch_mismatch(row, target_topic_path_labels, kc_id):
            rejected_cross_branch_mismatch += 1
            continue
    ...
    report = {
        ...
        "overlay_fallback_rejected_cross_branch_mismatch": rejected_cross_branch_mismatch,
        "overlay_fallback_weak_binding": weak_binding,
        "overlay_fallback_weak_binding_score_rescues": weak_binding_score_rescues,
    }
...
def main() -> int:
    ...
    for kid in sorted(rows_by_kc):
        ...
        packet = _sanitize_packet_strings(packet)
