...
def main() -> int:
    ...
    # 2026-07-26 fix: kc_registry_path may be a caller-scoped SUBSET of the real registry (see
    # step5_3_evidence_recalibrated_bridge.py's gap-KC scoping) - downstream consumers (e.g.
    # step_06_6_drafting_input_overlay) read this stage's own recorded upstream.kc_registry_path
    # as "the real, full KC registry" (a valid assumption before scoping existed, since this
    # stage always processed the full registry). Recording the scoped path there silently broke
    # that contract (confirmed real incident, run 20260725T225807Z_9e856df6, job 234940).
    # full_kc_registry_path_for_provenance lets a scoping caller supply the TRUE full registry
    # path for this field while kc_registry_path (above) still drives actual processing -
    # defaults to kc_registry_path itself when absent, so any caller that hasn't been updated
    # keeps its prior (always correct, since always unscoped) behavior unchanged.
    full_kc_registry_path_for_provenance = resolve_repo_path(
        repo_root, cfg["inputs"].get("full_kc_registry_path_for_provenance") or cfg["inputs"]["kc_registry_path"]
    )
    ...
    try:
        ...
        if acceptance_passed:
            set_payload = {
                ...
                "upstream": {
                    "kc_registry_path": rel_path(full_kc_registry_path_for_provenance, repo_root),
