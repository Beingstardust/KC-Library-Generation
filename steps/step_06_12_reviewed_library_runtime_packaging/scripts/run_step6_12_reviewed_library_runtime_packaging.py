...
from kc_l.kc.restarted_runtime_packaging import assemble_restarted_runtime_package
from kc_l.utils.json_io import read_json, write_json


DEFAULT_CONFIG = Path("steps/step_06_12_reviewed_library_runtime_packaging/resources/step6_12.slice8.yaml")
...
def choose_run_paths(runtime_root: Path, runs_root: Path, sets_root: Path) -> Dict[str, Path]:
    base_stamp = utc_stamp()
    suffix = 0
    while True:
        run_id = base_stamp if suffix == 0 else f"{base_stamp}_{suffix:02d}"
        runtime_dir = (runtime_root / run_id).resolve()
        audit_dir = (runs_root / f"{run_id}_step6_12").resolve()
        set_path = (sets_root / f"{run_id}_step6_12_kc_library_runtime_restarted_set.json").resolve()
        if not runtime_dir.exists() and not audit_dir.exists() and not set_path.exists():
            return {
                "run_id_step6_12": Path(run_id),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Package a reviewed-only restarted runtime library from the bounded Step 6.11 slice.")
...
def main() -> int:
    ...
    runtime_root = resolve_repo_path(output_cfg.get("processed_root") or "data/processed/kc_library_runtime_restarted")
    sets_root = resolve_repo_path(output_cfg.get("sets_root") or "data/processed/kc_library_runtime_restarted/_sets")
    runs_root = resolve_repo_path(output_cfg.get("runs_root") or "data/runs")

    step6_11_set_path = resolve_repo_path(
        input_cfg.get("step6_11_set_manifest")
        or "data/processed/kc_library_reviewed_restarted/_sets/2026-03-23_132444_step6_11_kc_library_reviewed_restarted_set.json"
    )
    ...
    run_paths = choose_run_paths(runtime_root, runs_root, sets_root)
    run_id = str(run_paths["run_id_step6_12"])
    ...
    result = assemble_restarted_runtime_package(
    ...
    runtime_summary = read_json(result.runtime_summary_path)
    ...
    set_manifest = {
        "schema_version": "1.0",
        "kind": "step6_12_kc_library_runtime_restarted_set",
        "set_id": f"{run_id}_step6_12_kc_library_runtime_restarted_set",
        "created_utc": now_utc_iso(),
        "run_id_step6_12": run_id,
        "artifacts": {
            "runtime_kc_library_jsonl": rel_path(result.runtime_library_path),
            "runtime_package_summary_json": rel_path(result.runtime_summary_path),
            "runtime_package_preview_md": rel_path(result.runtime_preview_path),
            "runtime_package_manifest_json": rel_path(result.runtime_manifest_path),
        },
        "upstream": {
            "step6_11_set_manifest_json": rel_path(step6_11_set_path),
            "frozen_reviewed_library_jsonl": rel_path(source_reviewed_library_path),
            "reviewed_library_summary_json": rel_path(reviewed_summary_path),
        },
        "slice": {
            "included_kcs": list(runtime_summary.get("included_kcs") or []),
            "scope_blank_kcs": list(runtime_summary.get("scope_blank_kcs") or []),
            "intentionally_blank_fields_preserved": list(runtime_summary.get("intentionally_blank_fields_preserved") or []),
            "sandbox_excluded_by_construction": bool(runtime_summary.get("sandbox_excluded_by_construction")),
        },
        "audit": {
    ...
    output_manifest = {
        "processed_outputs": build_output_manifest(runtime_dir),
    ...
    print(
        json.dumps(
            {
                "run_id": run_id,
                "runtime_dir": rel_path(runtime_dir),
                "audit_dir": rel_path(audit_dir),
                "runtime_entry_count": runtime_summary.get("runtime_entry_count"),
            },
            indent=2,
        )
    )
