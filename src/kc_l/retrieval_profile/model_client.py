...
def ollama_chat_json(
    *,
    ...
    options: Dict[str, Any] = {
        ...
        "top_p": 1,
        # 2026-08-02 fix, corrected 2026-08-02 (same day): only temperature/top_p/seed were
        # ever pinned here, leaving top_k/min_p/presence_penalty/repeat_penalty to each model's
        # own Modelfile defaults - a real fairness gap (same class of bug already fixed once for
        # step_06_7). First attempt pinned these to arbitrary "neutral" values (top_k=100,
        # repeat_penalty=1.0), but that changed gemma4:12b's own behavior versus its already-
        # audited/approved historical runs (Ablation 1/2), confirmed via a real side-by-side
        # diff of source_packet evidence counts for the same KC. Recalibrated: since gemma4:12b's
        # Modelfile only declares top_k=64 (confirmed via ollama show --verbose) and nothing
        # for min_p/presence_penalty/repeat_penalty, those three fall through to Ollama's own
        # DefaultOptions() (confirmed via Ollama's api/types.go source: RepeatPenalty=1.1,
        # PresencePenalty=0.0, MinP unset/0.0) - these are gemma4:12b's REAL effective values in
        # every historical run before this fix existed (the original code never sent these keys
        # at all, so they always silently fell through to exactly this chain). Pinning explicitly
        # to these confirmed values (not arbitrary neutral ones) makes every model swap at this
        # stage differ ONLY in the model weights while keeping gemma4:12b's own behavior
        # unchanged from its approved historical baseline.
        "top_k": 64,
        "min_p": 0.0,
        "presence_penalty": 0.0,
        "repeat_penalty": 1.1,
    }
    # 2026-07-27 fix: temperature=0 alone does not guarantee reproducible output across
    # separate Ollama inference calls (confirmed real incident this week - repeated fresh
