...
def build_role_target_contract(
    *,
    ...
    # Bug fix 2026-07-23: this used to be `if source_sparse_label_mismatch:`, which checks
    # dict truthiness rather than the actual boolean field inside it - _detect_source_sparse_
    # label_mismatch() (builder.py) always returns a non-empty dict (it has count/preview keys
    # even when the mismatch condition itself is False), so the old check fired unconditionally
    # for every KC and this rescue_hint carried zero signal. Confirmed via direct inspection:
    # 159/159 KCs in a real run showed this rescue_hint present regardless of their actual
    # retrieval state. Also surfaces the underlying audit dict as its own field below (not just
    # a hint with no data) so it can be inspected/queried directly from the serialized profile,
    # not just inferred from whether a hint fired.
    if source_sparse_label_mismatch and source_sparse_label_mismatch.get("source_sparse_label_mismatch"):
        rescue_hints.append({
    ...
    return {
        ...
        "rescue_hints": rescue_hints,
        "source_sparse_label_mismatch_audit": dict(source_sparse_label_mismatch) if source_sparse_label_mismatch else None,
...
# Lane-priority ladder (Phase 3.5 rank #16 / audit codebase-audit-20260805 item 17): the
# 10/20/30/40/50/60/70 (+90 unknown-lane fallback) values are literal, no config override, no
# dedicated calibration record. Investigated as part of this audit and left unchanged,
# documented as an accepted, reasoned default per the audit's own explicit fallback: read as
# policy, not syntax, the values themselves don't matter beyond their RELATIVE ordering (this is
# a priority ranking, not an additive score - only comparisons like a<b matter, so the specific
# gaps of 10 carry no independent meaning and there is nothing to "calibrate" beyond the
# ordering itself). The ordering is coherent and matches the lane names' own semantics:
# primary_core first, formula/procedure and example/boundary next (concrete content classes),
# bridge_context and context_or_guardrail after (support material), source_section_locator and
# rescue last (weakest, most speculative routes) - this decides which retrieval route wins when
# step5p emits multiple options for the same KC, and the ordering direction was not found to be
# in question, only whether specific numeric gaps needed justifying (they don't, given the
# comparison-only usage confirmed by reading every call site of this function).
...
# positive_support_condition (Phase 3.5 rank #17 / audit codebase-audit-20260805 item 18):
# live-traced this audit (grep across src/, steps/, scripts/, tests/) - written into every
# retrieval-strategy-plan item by this function and its callers below, but has NO downstream
# reader anywhere in the live codebase. This is the human-readable instruction Step5x would
# need to read to know it must independently verify target binding/source grounding before
# trusting a route's positive support - genuinely computed, genuinely never consumed, matching
# the audit's "computed and stored but not actually used" pattern.
#
# Deliberately NOT removed and NOT newly wired into Step5x admission in this pass: removal
# would discard real, still-relevant design intent for zero behavioral gain (it's dead weight,
# not a bug), while actually wiring it into evidence_stage_v3_scored_candidates.py's admission
# gates would be a genuine behavior change/new feature - its own scoped design-and-verify
# effort, not a safe same-day cleanup fix within this audit's 2-attempt rollback discipline.
# Documented explicitly per the audit's standing goal (never leave silently unexplained just
# because it hasn't caused a visible bug) rather than left as an unremarked dead field or
# force-wired without proper scoping.
