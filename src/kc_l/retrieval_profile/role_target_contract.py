...
def build_role_target_contract(
    *,
    ...
    # Test the boolean field inside the dict, never the dict itself.
    # _detect_source_sparse_label_mismatch (builder.py) always returns a non-empty dict - it
    # carries count/preview keys even when the mismatch condition is False - so a truthiness
    # check on the dict fires for every unit and makes this rescue_hint carry no signal at all.
    # The underlying audit dict is also surfaced as its own field below, rather than only as a
    # hint with no data, so it can be inspected directly from the serialized profile,
...
# Lane-priority ladder : the
# 10/20/30/40/50/60/70 (+90 unknown-lane fallback) values are literal, no config override, no
# dedicated calibration record.and left unchanged,
# documented as an accepted, reasoned default: read as
...
# Deliberately NOT removed and NOT newly wired into Step5x admission: removal
...
# Documented explicitly standing goal (never leave silently unexplained just
