...
DEFAULT_STEP67_RESPONSE_PARSE_MODE = "message_content_only"
...
def normalize_step67_model_profile(raw_profile: Mapping[str, Any] | None) -> Dict[str, Any]:
    ...
    profile["system_prefix"] = as_text(source.get("system_prefix"))
...
def _prepend_system_message(prepared: List[Dict[str, str]], prefix: Any) -> List[Dict[str, str]]:
    clean_prefix = as_text(prefix).strip()
    if not clean_prefix:
        return prepared
    ...
    for message in out:
        ...
        if clean_prefix not in current:
            message["content"] = f"{clean_prefix}\n\n{current}" if current else clean_prefix
        return out

    return [{"role": "system", "content": clean_prefix}, *out]
...
def apply_step67_model_profile_to_messages(
    ...
    prepared = _prepend_system_message(prepared, profile.get("system_prefix"))

    if (
        as_bool(profile.get("thinking_enabled"))
        and as_text(profile.get("thinking_activation_mode")).lower() == "system_prefix"
    ):
        prepared = _prepend_system_message(prepared, profile.get("thinking_system_prefix"))

    return prepared


def step67_model_profile_think_flag(profile: Mapping[str, Any]) -> bool | None:
    mode = as_text(profile.get("thinking_activation_mode")).lower()
    if mode == "off":
        return None
    if mode == DEFAULT_STEP67_THINKING_ACTIVATION_MODE:
