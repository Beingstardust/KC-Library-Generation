...
def _normalize_step67_drafting_policy_cfg(
    drafting_cfg: Dict[str, Any],
    selection_cfg: Dict[str, Any],
    *,
    drafting_field_prefix: str,
    selection_field_prefix: str,
) -> Dict[str, Any]:
    normalized = dict(drafting_cfg)

    selection_mode = as_text(selection_cfg.get("definition_generation_mode"))
    drafting_mode = as_text(normalized.get("definition_generation_mode"))
    if drafting_mode and selection_mode and drafting_mode != selection_mode:
        raise RuntimeError(
            "Conflicting Step 6.7 definition generation mode values at "
            f"{drafting_field_prefix}.definition_generation_mode={drafting_mode!r} and "
            f"{selection_field_prefix}.definition_generation_mode={selection_mode!r}."
        )
    if not drafting_mode and selection_mode:
        normalized["definition_generation_mode"] = selection_mode
        normalized["definition_generation_mode_field"] = (
            f"{selection_field_prefix}.definition_generation_mode"
        )
    elif drafting_mode:
        normalized["definition_generation_mode_field"] = (
            f"{drafting_field_prefix}.definition_generation_mode"
        )

    llm_budget = normalized.get("max_llm_calls_per_kc")
    if llm_budget is not None:
        normalized["max_llm_calls_per_kc_field"] = (
            f"{drafting_field_prefix}.max_llm_calls_per_kc"
        )
...
def normalize_runner_config(
    ...
    *,
    ...
    if isinstance(cfg.get("step6_7"), dict):
        ...
        drafting_cfg = _normalize_step67_drafting_policy_cfg(
            dict(step67_cfg.get("drafting_policy") or {}),
            selection_cfg,
            drafting_field_prefix="step6_7.drafting_policy",
            selection_field_prefix="step6_7.selection",
        )
    ...
    drafting_cfg = _normalize_step67_drafting_policy_cfg(
        dict(cfg.get("drafting") or {}),
        selection_cfg,
        drafting_field_prefix="drafting",
        selection_field_prefix="selection",
    )
