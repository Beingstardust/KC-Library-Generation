...
def validate_review_packets(packets: List[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    expected_counts_ok = (
        len(packets) == 165
        and unit_type_counter.get("kc", 0) == 144
        and unit_type_counter.get("topic", 0) == 21
        and review_mode_counter.get("segmentable_gap_review", 0) == 19
    )

    if not expected_counts_ok:
        issues.append("expected_counts_not_met")
