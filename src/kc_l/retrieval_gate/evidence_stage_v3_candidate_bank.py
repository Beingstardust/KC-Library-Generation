...
# Bibliography/reference-list entry shape: author-initial patterns ("Smith, J.:" / "Smith, J.,"),
# a year in parens, or journal/proceedings markers. Requires >=2 independent hits so a single
# incidental match (e.g. one stray "(2005)" inside real prose) doesn't false-trigger - confirmed
# live via a real corpus case (KC_DE_MISS_001: the top-scoring "candidate" for this KC's own
# alias phrase was citation #87's title, isolated from its authors/year by sentence
# segmentation - REFERENCE_LIKE_RE alone doesn't catch it since the isolated sentence carries
# none of "references/et al./doi").
CITATION_LIKE_RE = re.compile(
    r"(?:\b[A-Z][a-zA-Z\-]+,\s*[A-Z]\.(?:[A-Z]\.)?[,:]|"
    r"\(\d{4}\)|"
    r"\bet\s+al\.?\b|"
    r"\bvol\.?\s*\d|"
    r"\bpp\.?\s*\d|"
    r"\bjournal\s+of\b|\bproc(?:eedings)?\.?\b|\bieee\b|\bacm\b)",
    re.IGNORECASE,
)
...
def _looks_like_citation_row(views: Mapping[str, Any]) -> bool:
    """Bibliography/reference-list entry, independent of the window-widening logic (a candidate
    can be citation-shaped whether it came from a plain single-sentence match or a local-window
    expansion). Checked against `source_block_text`, not just `text` - the isolated sentence a
    reference-list title gets split into (e.g. "Missing value estimation for mixed-attribute
    data sets.") rarely carries author/year markers itself, but the full source block it was
    extracted from does (confirmed on a real corpus row: "Zhu, X., Zhang, S., ...: Missing value
    estimation for mixed-attribute data sets. IEEE Transactions ... (2011)").
    """
    combined = " ".join(
        str(views.get(key) or "") for key in ("text", "source_block_text", "heading_text")
    )
    return len(CITATION_LIKE_RE.findall(combined)) >= 2
...
def _is_unsafe_sentence_row(sentence_row: Mapping[str, Any], views: Mapping[str, Any]) -> bool:
    ...
    if CAPTION_RE.search(text.lower()) or CAPTION_RE.search(heading_text.lower()):
        return True
    if _looks_like_citation_row(views):
        return True
    return False
...
def _is_specific_anchor_match(matched_terms: Sequence[str]) -> bool:
    """A match is "specific" enough to safely drive window expansion only if at least one
    matched term is a genuine multi-word phrase (canonical name, multi-word alias, formula
    string) rather than a single generic token (e.g. "binary", "ratio", "induction" - real
    single-word registry aliases confirmed, via a live contamination audit, to anchor onto
    unrelated chapters that merely happen to share that one common word). Single-word matches
    still produce their own single-sentence STRUCTURAL_ANCHOR candidate elsewhere (low blast
    radius, unchanged) - they just never get to pull in a local window of neighbors.
    """
    for term in matched_terms:
        if len(_content_tokens(term, min_len=1)) >= 2:
            return True
    return False


def _local_block_window_indices(
    *,
    anchor_index: int,
    sentence_rows: Sequence[Mapping[str, Any]],
    max_extra_blocks_each_direction: int = 1,
) -> List[int]:
    """Replaces the old patch_id/reveal_group_id membership expansion (measured on the real
    corpus at a median 51-52 sentences per group, up to ~1000) with a genuinely LOCAL window:
    the anchor's own `block_id` (median 1 sentence, p90 4 - the real paragraph/bullet-item
    extraction unit) plus at most one adjacent block in each direction, stopping the instant a
    heading row or another block's own definitional lead sentence is crossed. This is what
    keeps e.g. an "Exhaustive Search:" anchor from absorbing the very next "Heuristic Search:"
    bullet in the same list - confirmed as a real, live risk via direct PDF inspection (Ch.7 p.5
    of the feature-selection source book has exactly this three-sibling-bullet structure).
    """
    n = len(sentence_rows)
    anchor_row = sentence_rows[anchor_index]
    anchor_block_id = _as_text(anchor_row.get("block_id"))
    anchor_doc_id = _as_text(anchor_row.get("doc_id"))
    indices = [anchor_index]

    for step in (-1, 1):
        i = anchor_index + step
        extra_blocks_crossed: set[str] = set()
        while 0 <= i < n:
            row = sentence_rows[i]
            if _as_text(row.get("doc_id")) != anchor_doc_id:
                break
            if bool(row.get("is_heading_like")):
                break
            block_id = _as_text(row.get("block_id"))
            if block_id and block_id != anchor_block_id:
                if block_id not in extra_blocks_crossed:
                    if bool(row.get("is_definition_like")):
                        break
                    if len(extra_blocks_crossed) >= max_extra_blocks_each_direction:
                        break
                    extra_blocks_crossed.add(block_id)
            indices.append(i)
            i += step
    ...
    *,
    ...
    for kc_context in selected_kc_context_rows:
        ...
        for source_row_index, sentence_row in enumerate(sentence_rows):
            ...
            region_indices = (
                _local_block_window_indices(anchor_index=source_row_index, sentence_rows=sentence_rows)
                if _is_specific_anchor_match(matched_terms)
        ...
        for source_row_index in source_window_indices:
            ...
            if _is_unsafe_sentence_row(sentence_row, views):
                ...
                continue
            shape_tags = _structural_shape_tags(sentence_row, views)
            # step5p already vetted this exact sentence as relevant for this KC (it's one of the
            # model's own recommended source windows) - always eligible for the tight local
            # window, no additional specificity gate needed here.
            region_indices = _local_block_window_indices(anchor_index=source_row_index, sentence_rows=sentence_rows)
            region_structural_count = 0
            for region_index in region_indices:
                region_row = sentence_rows[region_index]
                region_views = _surface_views(region_row)
...
def _context_only_locator_terms(
    row: Mapping[str, Any],
    guidance: Optional[Step5xProfileGuidance],
) -> set[str]:
    """Attempt 2 (reverted) checked `guidance.region_locator_payloads` directly and found nothing -
    that field is only ONE of three sources `build_retrieval_policy_plan()` merges into the final
    `region_locators` list (the other two - `query_atom_long_locator`/`lexical_query_long_locator`
    - come from ordinary lexical queries that separately qualify as "long locators"). Confirmed
    live (KC_CLF_UND_002's real scored_candidates.jsonl record) that "induction" and "learning
    algorithm" both carry `activation: "context_only"` in that final, fully-assembled list - so
    build the plan the same way the rest of the pipeline does, via `build_retrieval_policy_plan`,
    rather than reconstructing a partial view of it from one of its inputs.
    """
    unit_context = {
        "kc_id": _as_text(row.get("kc_id")),
        "knowledge_unit_id": _as_text(row.get("knowledge_unit_id") or row.get("kc_id")),
        "knowledge_unit_type": _as_text(row.get("knowledge_unit_type")) or "kc",
        "canonical_name": _as_text(row.get("canonical_name")),
    }
    plan = build_retrieval_policy_plan(unit_context, guidance)
    terms: set[str] = set()
    for locator in plan.region_locators:
        if not isinstance(locator, Mapping):
            continue
        if _as_text(locator.get("activation")) != "context_only":
            continue
        text = _as_text(locator.get("locator") or locator.get("term") or locator.get("text"))
...
def _suppress_context_only_locator_only_fallback_rows(
    rows: List[Dict[str, Any]],
    profile_guidance_lookup: Mapping[str, Step5xProfileGuidance],
) -> List[Dict[str, Any]]:
    """`source_surface_fallback` (build_source_surface_fallback_candidates) matches directly
    against a KC's raw registry `aliases`, with no awareness of which of those aliases step5p
    itself already flagged as context-only locators (see `_context_only_locator_terms`) rather
    than real positive-evidence query terms. Confirmed live: "A wrapper uses the learning
    algorithm itself to evaluate feature subsets." (Feature-Selection chapter) was admitted as
    drafting_core_evidence for KC_CLF_UND_002 "Querying Phase" purely via the alias "learning
    algorithm" - a term step5p's own guidance for this exact KC marks context-only. Drop any
    fallback-sourced candidate whose ENTIRE matched-term basis is context-only locators for its
    KC (a candidate that also matched via a real, non-context-only term is left untouched).
    """
    context_only_cache: Dict[str, set[str]] = {}
    kept: List[Dict[str, Any]] = []
    suppressed = 0
    for row in rows:
        kc_id = _as_text(row.get("kc_id"))
        matched_terms = _as_str_list(row.get("matched_surface_terms"))
        if not matched_terms:
            kept.append(row)
            continue
        if kc_id not in context_only_cache:
            context_only_cache[kc_id] = _context_only_locator_terms(row, profile_guidance_lookup.get(kc_id))
        context_only_terms = context_only_cache[kc_id]
        if not context_only_terms:
            kept.append(row)
            continue
        normalized_matches = [match_normalize(term) for term in matched_terms if match_normalize(term)]
        if normalized_matches and all(term in context_only_terms for term in normalized_matches):
            suppressed += 1
            continue
        kept.append(row)
    _ = suppressed  # available for stats wiring later; not logged (no module logger configured here)
    return kept
...
def _source_overlay_supplement_score(
    *,
    ...
    # Source-overlay supplement score ladder (Phase 3.5 rank #8 / audit codebase-audit-20260805
    # item 9): the text/source_block/heading base weights and the multi_token/acronym/relation/
    # branch_context/formula bonuses and short-sentence penalty below are literal constants, no
    # config override, no dedicated per-parameter calibration study or earlier commit record.
    #
    # Investigated as part of this audit and left unchanged, documented as an accepted, reasoned
    # default per the audit's own explicit fallback: the ladder is internally coherent (direct
    # sentence-text match outranks source-block recovery outranks heading-only, matching the same
    # text > source_block > heading priority order used by the config-driven
    # SURFACE_PRIORITY_EXACT table in retrieval_windowing/source_surface_fallback.py - not an
    # arbitrary, disconnected choice), and this generator is disabled by default
    # (direct_overlay_supplement.enabled defaults to false in
    # steps/step_05_x_evidence_stage_v3/resources/step5x_v3_candidate_bank.default.yaml) - it is
    # not on the always-live path the way the source_surface_fallback and structural-neighbor
    # mechanisms are, which is itself relevant context for how much verification weight this
    # specific ladder needs versus the already-corpus-validated live paths.
    text = _as_text(views.get("text"))
    score = 0.0
    reasons: List[str] = []
...
def _build_direct_overlay_supplement_candidates(
    *,
    ...
    for raw_row in supplement_rows:
        ...
        # Direct-overlay fallback score ladder (Phase 3.5 rank #9 / audit
        # codebase-audit-20260805 item 10): the 8.6/8.3/7.9/7.1/6.3 tier base scores and the
        # +0.55/+0.35/+0.6-capped/+0.4-capped bonuses below are literal constants, no config
        # override, no dedicated calibration record. This generator
        # (direct_overlay_supplement, distinct from the always-live source_surface_fallback
        # mechanism covered by items 11/12) is disabled by default
        # (steps/step_05_x_evidence_stage_v3/resources/step5x_v3_candidate_bank.default.yaml's
        # direct_overlay_supplement.enabled: false) and no real run config found in this repo
        # sets it to true - unlike source_surface_fallback, this ladder is not confirmed to have
        # ever fired in production, which is itself relevant context for how urgently it needs
        # calibration versus the always-live paths this session already verified corpus-wide.
        #
        # Investigated as part of this audit and left unchanged, documented as an accepted,
        # reasoned default: the ladder is internally coherent (exact canonical text > exact alias
        # > matched signal term > multi-token target > context-bound, a sensible specificity
        # ordering matching the same pattern used by the live SURFACE_PRIORITY_EXACT/_TOKEN/_HEAD
        # tables in retrieval_windowing/source_surface_fallback.py), just not independently
        # empirically calibrated the way the six role-score thresholds were.
...
def _build_clean_slate_candidate_rows(
    *,
    ...
    rows = _suppress_context_only_locator_only_fallback_rows(rows, profile_guidance_lookup)
