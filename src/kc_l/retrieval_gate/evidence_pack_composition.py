...
CAPTION_CUE_RE = re.compile(r"(?:^(?:figure|fig\.|table|algorithm)\b|\b(?:figure|fig\.|table|algorithm)\s+\d)")
TARGET_BINDING_STOPWORDS = {
    *STOPWORDS,
    "a",
    "an",
    "in",
...
QUALIFIER_CONNECTOR_RE = re.compile(r"\b(?:in|of|for|with|under|using|via|within|on|by)\b")
FORMULA_NOTATION_RE = re.compile(
    r"(?:\\(?:sum|frac|sqrt|log|begin|end|mathbb|mathbf|operatorname|widehat|overline)\b|"
    r"\b(?:argmax|argmin)\b|[∑Σ]|"
    r"\b(?:equation|eq\.|table)\s*\d+\b|"
    r"\bO\s*\(|"
    r"[A-Za-z][A-Za-z0-9_]*\s*=\s*[^=])"
)
DEFINITION_COMPLEMENT_RE = re.compile(r"\b(?:called|known as|termed|named|denoted|defined as|referred to as)\b")
GENERIC_SUBJECT_RE = re.compile(
    r"^(?:this|that|such|the)\s+(?:approach|method|process|procedure|measure|quantity|term|concept|criterion)\b"
)
PROMPT_CUE_RE = re.compile(r"^(?:describe|explain|compare|discuss|consider|identify|list|state|suppose)\b")
NEGATIVE_TARGET_RELATION_TEMPLATES = (
    r"\bnot\s+(?:a|an|the)?\s+{target}\b",
    r"\bneither\s+(?:a|an|the)?\s+{target}\b",
    r"\bwithin\s+the\s+neighbou?rhood\s+of\s+(?:a|an|the)?\s+{target}\b",
    r"\bfalls?\s+within\s+the\s+neighbou?rhood\s+of\s+(?:a|an|the)?\s+{target}\b",
    r"\bclose\s+enough\s+to\s+(?:a|an|the)?\s+{target}\b",
    r"\bbelongs?\s+to\s+(?:a|an|the)?\s+{target}\b",
    r"\bassociated\s+with\s+(?:a|an|the)?\s+{target}\b",
    r"\bsuch\s+as\s+(?:a|an|the)?\s+{target}\b",
)
FRAGMENT_PREFIXES = (
    "where ",
    "for ",
...
def _content_tokens(text: str) -> List[str]:
    ...
    for token in tokenize(match_normalize(text), min_len=2):
        ...
        if rooted and rooted not in TARGET_BINDING_STOPWORDS:
...
def _target_binding_profile(kc_row: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    canonical_tokens = tuple(_content_tokens(canonical_name))
    qualifier_tokens: List[str] = []
    canonical_norm = match_normalize(canonical_name)
    for match in QUALIFIER_CONNECTOR_RE.finditer(canonical_norm):
        qualifier_tokens.extend(_content_tokens(canonical_norm[match.end() :]))
    for group in re.findall(r"\(([^)]+)\)", canonical_name):
        qualifier_tokens.extend(_content_tokens(group))
    for acronym in re.findall(r"\b[A-Z]{2,}\b", canonical_name):
        qualifier_tokens.extend(_content_tokens(acronym))
    topic_path_labels = _topic_path_labels(kc_row)
    parent_topic_tokens = _content_tokens(_parent_topic_label(topic_path_labels))
    topic_tokens = [
        token
    ...
    return {
        ...
        "qualifier_tokens": unique_preserve_order(qualifier_tokens),
        "parent_topic_tokens": unique_preserve_order(parent_topic_tokens),
        "topic_tokens": unique_preserve_order(topic_tokens),
        "ambiguous_target": len(canonical_tokens) <= 2,
    }
...
def _combined_context_tokens(*parts: str) -> List[str]:
    values: List[str] = []
    for part in parts:
        values.extend(_content_tokens(part))
    return unique_preserve_order(values)


def _target_phrase_pattern(target_phrases: Sequence[str]) -> re.Pattern[str] | None:
    phrases = [phrase for phrase in unique_preserve_order(target_phrases) if phrase]
    if not phrases:
        return None
...
def _actual_formula_notation(candidate: Mapping[str, Any]) -> bool:
    text = _as_text(candidate.get("text"))
    if not text:
        return False
    if FORMULA_NOTATION_RE.search(text):
        return True
    if FORMULA_NOTATION_RE.search(_as_text(candidate.get("source_block_text"))):
        return True
    return False


def _definition_subject_alignment(candidate: Mapping[str, Any]) -> Tuple[bool, str]:
    text = _as_text(candidate.get("text"))
    if not text:
        return False, "definition_subject_missing"
    target_profile = _target_binding_profile(
        {
    ...
    target_pattern = _target_phrase_pattern(target_profile.get("target_phrases") or [])
    if target_pattern is None:
        return False, "definition_subject_missing"
    text_norm = match_normalize(text)
    link_match = DEFINITION_LINK_RE.search(text_norm)
    subject_text = text_norm[: link_match.start()].strip(" ,;:-") if link_match else text_norm
    subject_matches, subject_ratio, _ = _best_target_overlap(
        _content_tokens(subject_text),
        target_profile.get("phrase_token_sets") or [],
    )
    if target_pattern.search(subject_text) is not None:
        return True, "definition_subject_target_named"
    if subject_matches >= 2 and subject_ratio >= 0.5:
        return True, "definition_subject_target_overlap"
    if subject_matches >= 1 and subject_ratio >= 0.8:
        return True, "definition_subject_target_overlap"
    for phrase in target_profile.get("target_phrases") or []:
        escaped_phrase = re.escape(phrase)
        if re.search(
            rf"\b(?:called|known as|termed|named|denoted|defined as|referred to as)\b[^\.]{{0,80}}\b{escaped_phrase}\b",
            text_norm,
        ) is not None:
            return True, "definition_target_named_in_complement"
    if link_match is not None:
        complement_text = text_norm[link_match.end() :].strip(" ,;:-")
        if target_pattern.search(complement_text) is not None:
            if DEFINITION_COMPLEMENT_RE.search(text_norm) is not None and GENERIC_SUBJECT_RE.search(subject_text) is not None:
                return True, "definition_generic_subject_named_target"
    for phrase in target_profile.get("target_phrases") or []:
        escaped_phrase = re.escape(phrase)
        for template in NEGATIVE_TARGET_RELATION_TEMPLATES:
            if re.search(template.format(target=escaped_phrase), text_norm) is not None:
                return False, "definition_subject_negative_or_contrastive_target"
    if target_pattern.search(text_norm) is not None:
        return False, "definition_subject_mismatch"
    return False, "definition_subject_missing"


def _definition_statement_style(candidate: Mapping[str, Any]) -> bool:
    text_norm = match_normalize(str(candidate.get("text") or ""))
    if not text_norm:
        return False
    binding_reasons = set(_string_list((candidate.get("target_binding") or {}).get("binding_reasons")))
    if bool(candidate.get("definition_like", False)) and bool(candidate.get("fragmentary", False)):
        if {"target_phrase_recovered_from_source_block", "exact_target_phrase_in_heading", "heading_plus_local_target_overlap"} & binding_reasons:
            return True
    if DEFINITION_COMPLEMENT_RE.search(text_norm) is not None:
        return True
    link_match = DEFINITION_LINK_RE.search(text_norm)
    if link_match is None:
        return False
    link_token = link_match.group(0).strip()
    if link_token not in {"is", "are"}:
        return True
    complement = text_norm[link_match.end() :].lstrip(" ,;:-")
    return complement.startswith(
        (
            "a ",
            "an ",
            "the ",
...
def _global_candidate_usage(
    ...
    *,
    own_parent_topic_id: str = "",
) -> Tuple[int, int, int, int]:
    """Returns (text_kc_count, patch_kc_count, text_foreign_topic_kc_count,
    patch_foreign_topic_kc_count).

    The *_foreign_topic_kc_count fields are the sibling-aware companions to the raw *_kc_count
    fields, propagated for parity with evidence_stage_v3_scored_candidates.py's 2026-07-26 fix
    (_usage_for_candidate there carries the full rationale, incl. the confirmed KC_CLU_EVAL_002
    false positive it fixes): they count only recurring KCs whose own parent_topic_id differs
    from this candidate's target KC's parent_topic_id, rather than every recurring KC regardless
    of topic. Always <= the corresponding raw count (a strict subset), so using it instead can
    only relax over-triggering, never newly trigger a case the raw count would not already have
    caught - a one-directional, safe-by-construction change. Falls back to the raw count whenever
    own_parent_topic_id is unknown, since there is nothing to safely distinguish "foreign" from
    "sibling" without it.
    """
    ...
    text_kc_count = int(text_counts.get(text, 0))
    patch_kc_count = int(patch_counts.get(patch_id, 0))
    if not own_parent_topic_id:
        return text_kc_count, patch_kc_count, text_kc_count, patch_kc_count
    kc_id_to_parent = global_candidate_usage.get("kc_id_to_parent_topic_id") or {}
    text_kc_ids = (global_candidate_usage.get("text_to_kcs") or {}).get(text, [])
    patch_kc_ids = (global_candidate_usage.get("patch_to_kcs") or {}).get(patch_id, [])
    text_foreign_topic_kc_count = sum(
        1 for kid in text_kc_ids if kc_id_to_parent.get(kid) and kc_id_to_parent.get(kid) != own_parent_topic_id
    )
    patch_foreign_topic_kc_count = sum(
        1 for kid in patch_kc_ids if kc_id_to_parent.get(kid) and kc_id_to_parent.get(kid) != own_parent_topic_id
    )
    return text_kc_count, patch_kc_count, text_foreign_topic_kc_count, patch_foreign_topic_kc_count
...
def _candidate_target_binding(
    ...
    *,
    ...
    combined_context_tokens = set(_combined_context_tokens(text, source_block_text, heading_text))
    ...
    parent_topic_tokens = set(profile.get("parent_topic_tokens") or [])
    ...
    own_parent_topic_id = _parent_topic_id(kc_row, _topic_path_ids(kc_row, _topic_path_labels(kc_row)))
    text_kc_count, patch_kc_count, text_foreign_topic_kc_count, patch_foreign_topic_kc_count = _global_candidate_usage(
        candidate_row, global_candidate_usage, own_parent_topic_id=own_parent_topic_id
    )
    ...
    recurring_without_target_binding = bool(
        text_foreign_topic_kc_count >= recurring_text_threshold
        or (patch_foreign_topic_kc_count >= recurring_patch_threshold and patch_foreign_topic_kc_count > 0)
    )
    qualifier_hits = len(combined_context_tokens & qualifier_tokens)
    parent_topic_hits = len(combined_context_tokens & parent_topic_tokens)
    source_kc_id = str(candidate_row.get("source_kc_id") or "")
    source_kc_matches_target = bool(source_kc_id and source_kc_id == str(kc_row.get("kc_id") or ""))
    family_alignment = bool(qualifier_hits or parent_topic_hits or source_kc_matches_target)
    ...
    if qualifier_tokens and _binding_strength_value(binding_strength) >= _binding_strength_value("usable") and not family_alignment:
        offtarget_reasons.append("qualifier_or_parent_topic_missing")
        binding_strength = "weak"
    if (
        bool(profile.get("ambiguous_target", False))
        and _binding_strength_value(binding_strength) >= _binding_strength_value("usable")
        and not family_alignment
        and not text_exact
        and not heading_exact
        and (fragmentary or formula_like or procedure_like or caption_like or heading_like or not relation_like)
    ):
    ...
    return {
        ...
        "recurring_text_foreign_topic_kc_count": int(text_foreign_topic_kc_count),
        "recurring_patch_foreign_topic_kc_count": int(patch_foreign_topic_kc_count),
    }
...
def _candidate_view(
    ...
    prompt_like = bool(PROMPT_CUE_RE.search(match_normalize(text)))
    fragmentary = bool(
...
def _build_slot_item(
    ...
    *,
    ...
    return {
        ...
        "prompt_like": bool(candidate.get("prompt_like", False)),
...
def _candidate_risk_flags(candidate: Mapping[str, Any]) -> List[str]:
    ...
    if bool(candidate.get("prompt_like", False)):
    ...
    return bool(
        {
            ...
            "qualifier_or_parent_topic_missing",
            "parent_topic_alignment_missing_for_ambiguous_target",
        }
        & offtarget_reasons
    )
...
def _slot_eligibility(
    ...
    *,
    ...
    text_norm = match_normalize(str(candidate.get("text") or ""))
    generic_context_prefix = any(text_norm.startswith(prefix) for prefix in GENERIC_CONTEXT_PREFIXES)
    fragmentary_recoverable = bool(
        {
            "target_phrase_recovered_from_source_block",
            "exact_target_phrase_in_heading",
            "heading_plus_local_target_overlap",
        }
        & set(_string_list((candidate.get("target_binding") or {}).get("binding_reasons")))
    )
    ...
    if slot_name == "definition_kernel":
        if bool(candidate.get("formula_like", False)) and not allow_formula_primary:
            return False, "", ""
        if bool(candidate.get("formula_like", False)) and allow_formula_primary and not _actual_formula_notation(candidate):
            return False, "", ""
        if (
            ...
            or bool(candidate.get("prompt_like", False))
        ):
            return False, "", ""
        ...
        if not bool(candidate.get("relation_like", False)) and not bool(candidate.get("definition_like", False)):
            return False, "", ""
        if not _definition_statement_style(candidate):
            return False, "", ""
        subject_aligned, subject_reason = _definition_subject_alignment(candidate)
        if not subject_aligned:
            return False, "", ""
        if generic_context_prefix and subject_reason != "definition_target_named_in_complement":
            return False, "", ""
        if bool(candidate.get("fragmentary", False)) and not bool(candidate.get("definition_like", False)):
            if subject_reason not in {"definition_target_named_in_complement"}:
                return False, "", ""
    ...
    if slot_name == "explanatory_gloss":
        ...
        if bool(candidate.get("heading_like", False)):
            return False, "", ""
        if bool(candidate.get("caption_like", False)) and not same_region_anchor:
            return False, "", ""
        if generic_context_prefix and not same_region_anchor:
            return False, "", ""
        if bool(candidate.get("fragmentary", False)) and not same_region_anchor and not bool(candidate.get("relation_like", False)):
            if not fragmentary_recoverable:
                return False, "", ""
        if bool(candidate.get("genuine_sibling_signal", False)) and bool(candidate.get("relation_like", False)):
            subject_aligned, _ = _definition_subject_alignment(candidate)
            if not subject_aligned:
                return False, "", ""
    ...
    if slot_name == "formula_notation":
        if not bool(candidate.get("formula_like", False)):
            return False, "", ""
        if not _actual_formula_notation(candidate):
            return False, "", ""
    ...
    if slot_name == "scope_condition":
        ...
        if bool(candidate.get("prompt_like", False)):
            return False, "", ""
        if bool(candidate.get("heading_like", False)) or bool(candidate.get("caption_like", False)):
            return False, "", ""
        if bool(candidate.get("procedure_like", False)) and not bool(candidate.get("relation_like", False)) and not same_region_anchor:
            return False, "", ""
        if direct_target_bound:
...
def _context_completion_item(
    ...
    *,
    ...
    completion_binding = _candidate_target_binding(
        ...
        caption_like=bool(candidate.get("caption_like", False)),
        heading_like=False,
        relation_like=bool(candidate.get("relation_like", False)),
...
def _build_global_candidate_usage(
    *,
    ...
    # kc_id_to_parent_topic_id: sibling-aware fix parity with evidence_stage_v3_scored_candidates.py's
    # 2026-07-26 fix (see _global_candidate_usage's docstring). Computed identically to how
    # compose_evidence_pack derives each kc_row's own parent_topic_id elsewhere in this file.
    kc_id_to_parent_topic_id: Dict[str, str] = {}
    for kc_row in kc_rows:
        kc_id = str(kc_row.get("kc_id") or "")
        if kc_id and kc_id not in kc_id_to_parent_topic_id:
            topic_path_labels = _topic_path_labels(kc_row)
            topic_path_ids = _topic_path_ids(kc_row, topic_path_labels)
            kc_id_to_parent_topic_id[kc_id] = _parent_topic_id(kc_row, topic_path_ids)
    ...
    return {
        ...
        "text_to_kcs": {key: sorted(value) for key, value in text_to_kcs.items()},
        "patch_to_kcs": {key: sorted(value) for key, value in patch_to_kcs.items()},
...
def compose_evidence_pack(
    *,
    ...
    if not slots["definition_kernel"]:
        for item in explanatory_items:
            ...
            if not _item_is_natural_language_anchor(item):
                continue
            if bool(source_candidate.get("genuine_sibling_signal", False)) and not _definition_statement_style(source_candidate):
                continue
    ...
    context_source_ids_seen: set[str] = set()
    for slot_name in ("definition_kernel", "explanatory_gloss"):
        for slot_item in slots.get(slot_name) or []:
            ...
            if source_candidate is None:
                continue
            if source_candidate_id in context_source_ids_seen:
                continue
            ...
            context_source_ids_seen.add(source_candidate_id)
