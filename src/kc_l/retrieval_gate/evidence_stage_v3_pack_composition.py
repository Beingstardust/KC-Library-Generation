...
from kc_l.retrieval_gate.branch_scoping import (
    CROSS_BRANCH_FLAG,
...
PAGE_BRANCH_MAP_PATH = REPO_ROOT / "data/processed/branch_scoping/page_branch_map.json"
_PAGE_BRANCH_MAP_CACHE: Optional[Dict[str, Any]] = None


def _get_page_branch_map() -> Dict[str, Any]:
    """Lazily loaded once per process, cached. Missing file -> empty map (no-op, never crashes
    the pipeline) since this map is a supplementary quality signal, not a hard dependency."""
    global _PAGE_BRANCH_MAP_CACHE
    if _PAGE_BRANCH_MAP_CACHE is None:
        try:
            _PAGE_BRANCH_MAP_CACHE = load_page_branch_map(PAGE_BRANCH_MAP_PATH)
        except FileNotFoundError:
            _PAGE_BRANCH_MAP_CACHE = {}
    return _PAGE_BRANCH_MAP_CACHE


def _split_cross_branch_mismatches(
    items: List[Dict[str, Any]], target_topic_path_labels: List[str], kc_id: str
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Splits items into (kept, demoted). Demoted items get CROSS_BRANCH_FLAG appended to their
    own review_risk_flags plus a lane_demotion_reason, matching the existing demotion pattern
    used by _shapeaware_core_aligned_to_ordered_pack so downstream consumers (step6_7) see a
    consistent shape regardless of which check demoted an item.
    """
    page_branch_map = _get_page_branch_map()
    if not page_branch_map or not target_topic_path_labels:
        return items, []
    ...
    for raw in items:
        ...
        is_mismatch, detail = check_cross_branch_mismatch(
            page_branch_map,
            doc_id=doc_id,
            page_index=page_index,
            target_topic_path_labels=list(target_topic_path_labels),
            own_kc_id=kc_id,
        )
        if is_mismatch:
            item["lane_demotion_reason"] = "cross_branch_evidence_mismatch"
            item["cross_branch_evidence_detail"] = detail
            flags = _string_list(item.get("review_risk_flags"))
            flags.append(CROSS_BRANCH_FLAG)
...
def _limit_unique_role_items(
    ...
    *,
    ...
    out: List[Dict[str, Any]] = []
    total_chars = 0
    prior = [dict(item) for item in prior_selected]
    for item in items:
        item_dict = dict(item)
        if len(_dedupe_items([*prior, *out, item_dict])) != len(prior) + len(out) + 1:
            continue
        text = _as_text(item_dict.get("text") or item_dict.get("quote"))
        if out and max_chars > 0 and total_chars + len(text) > max_chars:
            continue
...
def compose_pack_for_kc(kc_id: str, kc_rows: Sequence[Mapping[str, Any]], cfg: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # Cross-branch scoping must happen HERE, before candidates are promoted into slots ->
    # ordered_pack, not just on the later shapeaware drafting_core_evidence/auxiliary_evidence
    # buckets - ordered_pack_for_drafting is what step6_7's select_kc_synthesis_evidence_from_
    # embedded_pack() actually reads FIRST (confirmed by tracing the real Separation case: its
    # wrong evidence was served from ordered_pack_for_drafting, not drafting_core_evidence -
    # filtering only the later bucket would have been a no-op on the actual drafted output).
    target_topic_path_labels = _string_list(first.get("topic_path_labels"))
    cross_branch_demoted_all: List[Dict[str, Any]] = []

    raw_positive_candidates = _positive_item_candidates(kc_rows)
    positive_candidates: Dict[str, List[Dict[str, Any]]] = {}
    for role, items in raw_positive_candidates.items():
        kept, demoted = _split_cross_branch_mismatches(list(items or []), target_topic_path_labels, kc_id)
        positive_candidates[role] = kept
        cross_branch_demoted_all.extend(demoted)

    selected_for_ordered_pack: List[Dict[str, Any]] = []
    for role in POSITIVE_ROLES:
        slots[role] = _limit_unique_role_items(
            positive_candidates.get(role) or [],
            limit=int(comp[f"max_{role}"]),
            max_chars=int(comp["max_ordered_pack_chars"]),
            prior_selected=selected_for_ordered_pack,
        )
        selected_for_ordered_pack.extend(slots[role])

    positive_count = sum(len(slots.get(role) or []) for role in POSITIVE_ROLES)
    raw_aux_candidates = _auxiliary_item_candidates(kc_rows)
    aux_candidates, aux_demoted = _split_cross_branch_mismatches(
        list(raw_aux_candidates or []), target_topic_path_labels, kc_id
    )
    cross_branch_demoted_all.extend(aux_demoted)
    guard_candidates = _guardrail_item_candidates(kc_rows)
    slots["context_completion"] = _limit_unique_role_items(
        aux_candidates,
        limit=int(comp["max_context_completion"]),
        max_chars=int(comp["max_ordered_pack_chars"]),
        prior_selected=selected_for_ordered_pack,
    )
    selected_for_ordered_pack.extend(slots["context_completion"])
    ...
    shape_aligned_core, demoted_shapeaware_core = _shapeaware_core_aligned_to_ordered_pack(
        raw_shapeaware_core,
        ordered_pack,
    )
    drafting_core_evidence, demoted_cross_branch_core = _split_cross_branch_mismatches(
        shape_aligned_core, target_topic_path_labels, kc_id
    )
    auxiliary_evidence, demoted_cross_branch_aux = _split_cross_branch_mismatches(
        _as_list(shapeaware_shadow.get("auxiliary_evidence")), target_topic_path_labels, kc_id
    )
    cross_branch_demoted_all.extend(demoted_cross_branch_core)
    cross_branch_demoted_all.extend(demoted_cross_branch_aux)
    review_needed_evidence = _dedupe_items([
        *_as_list(shapeaware_shadow.get("review_needed_evidence")),
        *demoted_shapeaware_core,
        *cross_branch_demoted_all,
