...
ORDERED_EVIDENCE_BLOCKER_FLAGS = {
    ...
    # 2026-07-27: "recurring_global_candidate" was wired in here earlier tonight (see git blame /
    # prior session log for the full original comment) after confirming it was computed but never
    # consulted, motivated by a real catch (KC_FSEL_GEN_001/002 sharing identical generic
    # "stopping criteria" text). REVERTED after a real A/B audit (301 live candidates blocked
    # purely by a wired flag despite passing role_score + role_eligibility) showed this flag has
    # a meaningful false-positive rate on real data: e.g. "Rand Index and Jaccard Coefficient are
    # external validation measures..." (score 9.8, near-max) and "In particular, we can consider
    # the null hypothesis that there is no cluster structure..." (score 10.4) both blocked solely
    # by this flag despite being substantive, on-topic, high-scoring content. Root cause: the
    # underlying mechanism already matches on identical text (raw_text_hash/normalized_text/
    # patch_id, sibling-aware via *_foreign_topic_kc_count - see evidence_stage_v3_scored_
    # candidates.py's _usage_for_candidate) - confirmed directly against real data (the Rand/
    # Jaccard sentence recurs with byte-identical raw_text_hash across 8 KCs spanning 3 distinct
    # parent_topic_id branches). So "require near-identical/identical text matching instead of
    # concept co-occurrence" (the originally proposed second attempt) is not a fix - it's already
    # how this works, and still produces this false-positive class, because a single sentence can
    # legitimately be genuine supporting content for several closely-related sibling KCs at once
    # (e.g. one sentence naming two sibling evaluation metrics together). Per the rollback rule
    # (2 attempts max on a single fix), not attempting a third iteration here - reverted to the
    # pre-2026-07-26 state (flag remains computed and visible in risk_flags/warnings for audit
    # purposes, just not gating admission). A real fix would need to distinguish "verbatim
    # generic filler reused across unrelated KCs" from "a substantive sentence that legitimately
    # names multiple sibling concepts," which needs a different signal than recurrence count
    # alone - out of scope for a same-night patch.
}
...
def _procedure_structural_anchor_allowed(
    row: Mapping[str, Any], text: str, text_tokens: "set[str]"
) -> bool:
    """Structural, vocabulary-free escape for genuinely coherent procedural evidence -
    mirrors the existing formula-anchor conditional pattern (content-class flag AND an
    independent target-binding signal) rather than widening the predicate-word list.

    The generic has_predicate check below is tuned for passive/copula phrasing ("X is
    performed") and misses active-voice, domain-specific verbs ("recomputes", "partitions",
    ...) that ordinary procedural writing uses instead. Rather than growing that whitelist
    per-domain, this checks for independent, already-computed structural signals: the row is
    role-eligible as procedural content, it is lexically bound to the target KC, it is long
    enough to not be a caption fragment, and it carries a leading sequence/step marker - a
    near-universal English procedural-discourse pattern (recipes, protocols, legal steps,
    algorithms alike), not subject-matter vocabulary.
    """
    role_eligibility = _dict_value(row, "role_eligibility")
    if not bool(role_eligibility.get("example_or_procedure")):
        return False
    candidate_quality = _dict_value(row, "candidate_quality")
    if not bool(candidate_quality.get("target_bound_positive_support")):
        return False
...
def _derived_ordered_evidence_blockers(row: Mapping[str, Any], role: str) -> list[str]:
    ...
    # A structurally anchored row sits in the same source patch as an already-validated
    # target-bound positive for the same unit, which is a stronger guarantee than the profile
    # route or exact-target restatement this check demands.
    if formula_neighbor_without_profile_or_exact_target and not row.get("structural_role_anchor"):
    ...
    # caption_like_positive_evidence_without_predicate gate (Phase 3.5 rank #7 / audit
    # codebase-audit-20260805 item 8): the <=14-token cutoff itself is a literal, no config
    # override, no dedicated calibration record for the number specifically. But this exact gate
    # (not merely the surrounding function) had a real, recent, corpus-wide-verified fix pass
    # (commit 60ed779, "Add structural procedure-anchor escape to the caption/is_procedure_like
    # blockers"): confirmed a real false-positive class (active-voice procedural sentences like
    # "Step 4 recomputes the centroids..." wrongly blocked because has_predicate's verb whitelist
    # only recognized passive/copula phrasing), added the
    # _procedure_structural_anchor_allowed() escape below rather than widening the verb list, and
    # verified corpus-wide against the real 159-KC baseline: 0 route changes, 0 lost items, +1
    # correctly newly-admitted item, 0 new contamination.
    #
    # Investigated as part of this audit and left unchanged, documented as an accepted, reasoned
    # default: the gate only fires when FOUR conditions hold simultaneously (positive-prose role,
    # missing subject, no predicate verb, AND short text) - a real definition sentence almost
    # always has an explicit predicate by grammatical necessity, so this is closer to "no verb no
    # subject, this reads like a caption" than "any short sentence is suspect." The 14-token cutoff
    # itself hasn't been isolated and independently calibrated the way commit 60ed779 calibrated
    # the escape-hatch conditions, but it operates inside a gate that has had real, recent,
    # corpus-wide empirical scrutiny - not a bare, never-examined magic number.
    if (
        role_is_positive_prose
        and subject_missing
        and not has_predicate
        and len(text_tokens) <= 14
        and not _procedure_structural_anchor_allowed(row, text, text_tokens)
    ):
