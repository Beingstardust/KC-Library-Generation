...
META_GUIDANCE_RE = re.compile(
    r"\b(?:common\s+mistakes?|mistakes?|pitfalls?|caution|warning|avoid|be\s+careful|should\s+not|must\s+not)\b"
)
...
def _profile_route_evaluation(candidate_row: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    canonical_named = bool(alignment.get("exact_name_phrase"))
    ...
    # If Step 5p supplied a route contract, non-canonical positive evidence must
    # be supported by at least one positive route match. Otherwise broad rows
    # found near a context locator can leak into the leaf-KC evidence pack.
    route_contract_positive_required = bool(
        route_contract_present
        and positive_route_count == 0
        and not canonical_named
    )
    # Route-level missing support blocks positive support only when the route is
    # the apparent source of the target binding. Direct canonical-name evidence
    # remains eligible through normal Step 5x gates.
    block_missing_support = bool(missing_support_without_positive and not canonical_named)
    reason = ""
...
def _candidate_target_binding(
    ...
    *,
    ...
    recurring_without_target = bool(
        usage["raw_text_hash_kc_count"] >= int(thresholds["global_recurring_text_kc_threshold"])
        or usage["normalized_text_kc_count"] >= int(thresholds["global_recurring_text_kc_threshold"])
        or (
            usage["patch_kc_count"] >= int(thresholds["global_recurring_patch_kc_threshold"])
            and usage["patch_kc_count"] > 0
        )
    )
...
def _definition_subject_alignment(candidate_row: Mapping[str, Any], profile: Mapping[str, Any]) -> Tuple[str, List[str]]:
    ...
    if target_match_in_text is not None:
        ...
        return "mismatch", ["definition_subject_mismatch"]
    return "missing", ["definition_subject_missing"]
...
def _meta_guidance_signal(candidate_row: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    score = 0.0
    reasons: List[str] = []
    if META_GUIDANCE_RE.search(lower):
        score += 2.0
        reasons.append("meta_guidance_or_common_mistake_surface")
...
def _reference_like_signal(candidate_row: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if BIBLIOGRAPHY_HEADING_RE.search(heading):
        bibliography_score += 2.0
        reasons.append("bibliography_heading")
...
def _usage_for_candidate(candidate_row: Mapping[str, Any], global_candidate_usage: Mapping[str, Any] | None) -> Dict[str, int]:
    ...
    return {
        "raw_text_hash_kc_count": len(raw_map.get(raw_hash, [])) if raw_hash else 0,
        "normalized_text_kc_count": len(text_map.get(norm_text, [])) if norm_text else 0,
        "patch_kc_count": len(patch_map.get(patch_id, [])) if patch_id else 0,
    }
...
def _contamination_signals(
    ...
    *,
    ...
    usage = _usage_for_candidate(candidate_row, global_candidate_usage)
    if (
        usage["raw_text_hash_kc_count"] >= int(default_scoring_cfg(cfg)["global_recurring_text_kc_threshold"])
...
def _risk_flags(
    *,
    ...
    if (
        usage["raw_text_hash_kc_count"] >= int(default_scoring_cfg(cfg)["global_recurring_text_kc_threshold"])
        or usage["patch_kc_count"] >= int(default_scoring_cfg(cfg)["global_recurring_patch_kc_threshold"])
    ):
...
def default_scoring_cfg(cfg: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "min_definition_score": float(thresholds.get("min_definition_score", 3.9)),
        "min_explanatory_score": float(thresholds.get("min_explanatory_score", 3.1)),
        "min_scope_score": float(thresholds.get("min_scope_score", 3.0)),
        ...
        "min_example_or_procedure_score": float(thresholds.get("min_example_or_procedure_score", 3.0)),
