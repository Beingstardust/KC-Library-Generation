...
from kc_l.retrieval_gate.evidence_pack_composition import (
    build_evidence_pack_membership_index,
...
# 2026-07-26: step_05_3_evidence_recalibrated dismantled as a live pipeline stage - it caused
# four distinct incidents in one night (hierarchy-path resolution, acceptance-threshold scaling,
# provenance-pointer poisoning, and this module's own hard step5_3-candidate requirement) each
# time a narrower fix revealed the next, a strong signal the retrofit (scoping it to step_05x's
# gap KCs) conflicted with more of the existing contract than originally scoped. step_05x_kc_
# evidence_stage_v3/topic_05x_evidence_stage_v3's own embedded pack (evidence_pack_row below) is
# now this module's sole evidence source, matching what step_06_7's own build_kc_packet() already
# treats as authoritative (see that module's select_kc_synthesis_evidence_from_embedded_pack) -
# step_05_3's remaining unique capability (a full sentence-corpus + baseline-step5_2-candidate
# cross-encoder reranking pass, genuinely different from step_05x's profile-guided discovery) is
# not yet ported into step_05x itself; until it is, a KC step_05x found zero evidence for
# produces zero overlay records here - an honest gap, not a crash or diluted content.
SOURCE_STEP_KIND = "step5x_v3_kc_evidence_stage"
...
def _evidence_items_from_pack_row(evidence_pack_row: Mapping[str, Any]) -> List[Dict[str, Any]]:
    """Adapt step_05x_kc_evidence_stage_v3/topic_05x_evidence_stage_v3's own embedded pack items
    (ordered_pack_for_drafting + review_needed_evidence) into the "evidence_item" shape
    _candidate_view_from_evidence()/_record_from_evidence() expect. Both stages trace back to
    the same step4_5_sentence_overlay row schema step_05_3 used to (doc_id/block_id/page_index/
    layer/bbox/sentence_id/sent_idx/char_start/char_end/reveal_group_id/patch_id/patch_heading/
    source_block_text all match field-for-field, confirmed against real pack output 2026-07-26) -
    the one real gap is the quote-text field name (step_05x calls it "quote", not "snippet");
    everything else is a straight passthrough plus the missing keys _candidate_view_from_evidence
    already defaults safely (patch_type/patch_span_source/text_source/reveal_canonical_page_index).
    """
    items: List[Dict[str, Any]] = []
    for source_list in ("ordered_pack_for_drafting", "review_needed_evidence"):
        for raw in evidence_pack_row.get(source_list) or []:
            if not isinstance(raw, Mapping):
                continue
            adapted = dict(raw)
            adapted["snippet"] = str(
                raw.get("snippet") or raw.get("quote") or raw.get("candidate_sentence_text") or raw.get("text") or ""
            )
...
EVIDENCE_PACK_PAYLOAD_COPIED_KEYS = {
    "pack_version",
    "pack_quality",
    "slots",
...
EVIDENCE_PACK_METADATA_KEY_HINTS = (
    "sidecar",
    "overlay",
    "lineage",
    "policy",
...
def _evidence_pack_metadata_sidecars(evidence_pack_row: Mapping[str, Any]) -> Dict[str, Any]:
    sidecars: Dict[str, Any] = {}
    for key, value in evidence_pack_row.items():
        key_s = str(key)
        key_l = key_s.lower()
        if key_s in EVIDENCE_PACK_PAYLOAD_COPIED_KEYS:
            continue
        if not any(hint in key_l for hint in EVIDENCE_PACK_METADATA_KEY_HINTS):
            continue
        if isinstance(value, (dict, list, str, int, float, bool)) or value is None:
            sidecars[key_s] = value
    return sidecars


def _evidence_pack_payload(
    *,
    evidence_pack_row: Optional[Mapping[str, Any]],
    pack_candidate_id: str,
    pack_membership_index: Optional[Mapping[str, Mapping[str, Any]]],
) -> Dict[str, Any]:
    if not evidence_pack_row:
        return {
            ...
            "review_needed_evidence": [],
            "rejected_false_positive_evidence": [],
            "evidence_lane_semantics": evidence_lane_semantics({}),
            "provenance_sidecar": {},
            "source_evidence_pack_row_metadata_sidecars": {},
            "evidence_pack_membership": {
                "pack_candidate_id": pack_candidate_id,
                "selected_for_pack": False,
                "selected_for_drafting_core": False,
                "slot_roles": [],
                "slot_positions": {},
                "ordered_pack_positions": [],
    ...
    membership = dict((pack_membership_index or {}).get(pack_candidate_id) or {})
    lane_membership = dict(evidence_lane_membership_index(evidence_pack_row).get(pack_candidate_id) or {})
    slot_roles = [str(item) for item in membership.get("slot_roles") or []]
    lane_roles = [str(item) for item in lane_membership.get("lanes") or []]
    return {
        "evidence_pack_available": True,
        "evidence_pack_version": str(evidence_pack_row.get("pack_version") or ""),
        "evidence_pack_quality": dict(evidence_pack_row.get("pack_quality") or {}),
        "evidence_pack_slots": dict(evidence_pack_row.get("slots") or {}),
        "ordered_pack_for_drafting": list(evidence_pack_row.get("ordered_pack_for_drafting") or []),
        "drafting_core_evidence": list(evidence_pack_row.get("drafting_core_evidence") or []),
        "auxiliary_evidence": list(evidence_pack_row.get("auxiliary_evidence") or []),
        "review_needed_evidence": list(evidence_pack_row.get("review_needed_evidence") or []),
        "rejected_false_positive_evidence": list(evidence_pack_row.get("rejected_false_positive_evidence") or []),
        "evidence_lane_semantics": dict(evidence_pack_row.get("evidence_lane_semantics") or evidence_lane_semantics(evidence_pack_row)),
        "provenance_sidecar": dict(evidence_pack_row.get("provenance_sidecar") or {}),
        "source_evidence_pack_row_metadata_sidecars": _evidence_pack_metadata_sidecars(evidence_pack_row),
        "evidence_pack_membership": {
            "pack_candidate_id": pack_candidate_id,
            "selected_for_pack": bool(slot_roles or list(membership.get("ordered_pack_positions") or [])),
            "selected_for_drafting_core": bool(lane_membership.get("selected_for_drafting_core", False)),
            "slot_roles": slot_roles,
            "lane_roles": lane_roles,
            "slot_positions": dict(membership.get("slot_positions") or {}),
            "ordered_pack_positions": [int(item) for item in membership.get("ordered_pack_positions") or []],
            "drafting_core_positions": [int(item) for item in lane_membership.get("drafting_core_positions") or []],
            "review_needed_positions": [int(item) for item in lane_membership.get("review_needed_positions") or []],
            "rejected_false_positive_positions": [int(item) for item in lane_membership.get("rejected_false_positive_positions") or []],
        },
    }
...
def _record_from_evidence(
    *,
    ...
    canonical_name = str(kc_row.get("canonical_name") or (evidence_pack_row or {}).get("canonical_name") or "")
    aliases = [str(item) for item in kc_row.get("aliases") or (evidence_pack_row or {}).get("aliases") or []]
    hierarchy_context = [
        str(item)
    ...
    for _legacy_alignment_key in list(alignment_breakdown.keys()):
        if str(_legacy_alignment_key).startswith("se" + "ed"):
            alignment_breakdown.pop(_legacy_alignment_key, None)
    ...
    pack_candidate_id = build_pack_candidate_id(
        str(kc_row.get("kc_id") or ""),
        source_candidate_index,
        evidence_item,
    )
    evidence_pack_payload = _evidence_pack_payload(
        evidence_pack_row=evidence_pack_row,
        pack_candidate_id=pack_candidate_id,
        pack_membership_index=pack_membership_index,
    )
...
def build_overlay_records(
    *,
    ...
    evidence_packs_by_kc: Optional[Mapping[str, Mapping[str, Any]]] = None,
    provenance_index: Mapping[str, Any],
    source_set_id: str,
    source_run_id: str,
    layer_preference: Sequence[str],
    ...
    evidence_pack_available_count = 0

    for kc_row in kc_rows:
        kc_id = str(kc_row.get("kc_id") or "")
        review_queue_row = review_queue_by_kc.get(kc_id)
        evidence_pack_row = dict((evidence_packs_by_kc or {}).get(kc_id) or {})
        # 2026-07-26: evidence source is step_05x/topic_05x's own embedded pack directly, not a
        # separate step_05_3 candidate set (see this module's SOURCE_STEP_KIND comment) - a KC
        # step_05x found nothing for legitimately produces zero records here, not a crash.
        evidence_rows = _evidence_items_from_pack_row(evidence_pack_row)
        pack_membership_index = (
            build_evidence_pack_membership_index(evidence_pack_row)
            if evidence_pack_row
            else {}
        )
        evidence_pack_available_count += int(bool(evidence_pack_row))
    ...
    stats = {
        ...
        "evidence_pack_available_count": int(evidence_pack_available_count),
    }
