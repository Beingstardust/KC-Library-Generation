...
OUT_OF_SCOPE_NOTE = (
    "Held KCs remain intentionally excluded from this session and are not reviewer-ready."
)
RESTARTED_ONLY_NOTE = (
...
def _session_bucket(packet: Mapping[str, Any]) -> str:
    if _scope_gap(packet):
        return "scope_gap_review"
    if _has_caution(packet):
        return "caution_review"
    return "clean_review"


def _session_bucket_rank(packet: Mapping[str, Any]) -> tuple[int, int, str]:
    bucket = _session_bucket(packet)
    bucket_rank = {"scope_gap_review": 0, "caution_review": 1, "clean_review": 2}[bucket]
    priority_rank = {"moderate_support": 0, "high_support": 1}.get(
        _as_text((packet.get("review_priority") or {}).get("bucket")),
        9,
    )
    return bucket_rank, priority_rank, _as_text(packet.get("kc_candidate_id"))
...
def _build_manual_inspection_bundle(review_packet_dir: Path, packets: Sequence[Mapping[str, Any]], summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "- The excluded held KCs are out of scope for this session and should not be treated as missing packets.",
        "",
        "## Packet Order",
        "",
    ]
...
def _build_real_reviewer_session_md(review_packet_dir: Path, manifest: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Held KCs are intentionally excluded from this session.",
        "",
        "## Included Packets",
        "",
    ]
    ...
    current_bucket = None
    bucket_titles = {
        "scope_gap_review": "Scope-Gap Reviewer-Editable Cases",
        "caution_review": "Caution-Flag Cases",
        "clean_review": "Clean Ready Cases",
    }
    for item in order:
