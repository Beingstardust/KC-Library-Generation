...
def _first_prov_entry(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Docling's own JSON schema stores provenance as a LIST of spans (one per
    physical page a text/table/picture item touches - e.g. a paragraph that
    straddles a page break), not a single dict:
      "prov": [{"page_no": 51, "bbox": {...}, "charspan": [0, 64]}]
    A block is anchored to the page its first provenance span reports.
    """
    prov = item.get("prov") or item.get("provenance")
    if isinstance(prov, dict):
        return prov
...
def _guess_page_index(item: Dict[str, Any]) -> Optional[int]:
    ...
    prov_entry = _first_prov_entry(item)
    if prov_entry is not None:
        # Docling's own "page_no" is 1-indexed; every other layer in this
        # pipeline (pymupdf, mineru) uses 0-indexed page_index, so convert
        # here to keep the field's meaning consistent across layers.
        v = prov_entry.get("page_no")
        if isinstance(v, int):
            return v - 1
        for k in ["page_idx", "page_index", "page"]:
            v = prov_entry.get(k)
            if isinstance(v, int):
                return v
...
def run_docling_cli(
    ...
    # Confirmed real, functional CLI option (docling/cli/main.py: device: Annotated[
    # AcceleratorDevice, typer.Option(...)], wired into AcceleratorOptions(device=device)) -
    # unlike MinerU's --device, which is silently ignored (see run_mineru_cli/MINERU_DEVICE_MODE
    # below). Valid values: auto|cpu|cuda|mps|xpu.
    cmd += ["--device", device]
...
def _docling_items_to_blocks(
    ...
    *,
    ...
    for i, item in enumerate(items):
        ...
        # `i` is only unique within a single chunk's item list. For chunked
        # PDFs (see _plan_docling_pdf_chunks), parse_docling_json_to_blocks
        # is called once per chunk, so without the chunk index folded in,
        # every chunk's texts:0, texts:1, ... would collide on the same
        # block_id, making block identity ambiguous document-wide.
        chunk_index = chunk_meta.get("chunk_index") if chunk_meta is not None else None
        if isinstance(chunk_index, int):
            block_id = f"{doc_id}:docling:chunk{chunk_index:03d}:{top_key}:{i}"
        else:
...
def extract_docling(
    ...
    common_cli_kwargs = {
        ...
        "artifacts_path": cfg.get("artifacts_path", ""),
