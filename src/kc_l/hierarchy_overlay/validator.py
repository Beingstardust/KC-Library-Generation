...
def validate_overlay(
    *,
    ...
    for node in nodes:
        ...
        if node.node_type != "leaf" and len(node.child_hier_node_ids) != node.raw_child_count:
            _issue(
                issues,
                level="ERROR",
                code="NONLEAF_CHILD_COUNT_MISMATCH",
                message=(
                    "Non-leaf overlay node child count "
                    f"({len(node.child_hier_node_ids)}) does not match its raw child count "
                    f"({node.raw_child_count})."
                ),
                node=node,
            )
    ...
    nonleaf_child_count_mismatches = sum(1 for issue in issues if issue.code == "NONLEAF_CHILD_COUNT_MISMATCH")
    nonleaf_identity_audit = {
        ...
        "nonleaf_child_count_mismatches": nonleaf_child_count_mismatches,
    }
    ...
    checks = {
        ...
        "nonleaf_child_count_ok": nonleaf_child_count_mismatches == 0,
    }
