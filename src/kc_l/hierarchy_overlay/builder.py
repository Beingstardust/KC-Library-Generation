...
def infer_node_type(
    *,
    ...
    if raw_kc_flag or not raw_has_children:
        basis = "raw_kc_flag" if raw_kc_flag else "terminal_without_children"
        return "leaf", basis
...
def build_overlay(
    *,
    ...
    def recurse(node: Mapping[str, Any], source_hierarchy_path: list[str], parent_hier_node_id: str | None) -> OverlayNode:
        ...
        kc_id = _clean_optional_str(node.get("kc_id")) if raw_kc_flag else None
        hier_node_id = (
            f"kc::{kc_id}"
            if kc_id
