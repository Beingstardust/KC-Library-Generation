...
def _ollama_executable() -> str:
    """Return the Ollama executable configured for this runtime.

    The HPC cluster uses an upgraded Ollama binary that may not be on PATH. Keep the
    fallback to "ollama" so the utility remains portable outside that environment.
    """
    return os.environ.get("OLLAMA_BIN") or "ollama"
...
def _candidate_json_payloads(
    ...
    *,
    ...
    for candidate_fragment in list(candidates):
        extracted = _extract_first_balanced_json_object(candidate_fragment)
        add(extracted)
...
def list_ollama_models() -> List[str]:
    proc = subprocess.run([_ollama_executable(), "list"], capture_output=True, text=True, timeout=30)
...
def ollama_show(name: str) -> str:
    proc = subprocess.run([_ollama_executable(), "show", name], capture_output=True, text=True, timeout=30)
    if proc.returncode != 0:
        return ""
