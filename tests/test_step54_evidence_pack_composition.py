...
def _kc_row(
    ...
    *,
    ...
    parent_hier_node_id: str = "topic-root",
) -> dict[str, object]:
...
def _ordered_roles(pack: dict[str, object]) -> list[str]:
    return [str(item["role"]) for item in pack["ordered_pack_for_drafting"]]
...
def test_handling_missing_values_requires_nb_or_parent_alignment():
    kc_row = _kc_row(
        "KC_CLF_NB_011",
        "Handling Missing Values in NB",
        aliases=["missing values"],
        path=["Classification", "Naive Bayes", "Handling Missing Values in NB"],
    )
    generic_missing = _candidate(
        "Unfortunately, since some of the patients have missing values for this field, it is impossible to say whether a 0 is real or missing.",
        patch_heading="Data Miner: No.",
        page_heading_norm="Data Miner: No.",
        relation_like=False,
    ...
    decision_tree = _candidate(
        "Figure 3.18 shows three different ways for handling missing values in a decision tree classifier.",
        block_id="doc-target:block:002",
        sent_idx=1,
        patch_heading="Methods for handling missing attribute values in decision tree classifier",
        page_heading_norm="Methods for handling missing attribute values in decision tree classifier",
        relation_like=False,
        exact_alias_phrase=True,
        alias_token_hits=2,
        name_or_alias_token_hits=2,
    ...
    assert pack["pack_quality"]["route"] != "standard_drafting"
    assert generic_missing["snippet"] not in _positive_slot_texts(pack)
    assert decision_tree["snippet"] not in _positive_slot_texts(pack)


def test_core_point_border_definition_cannot_be_definition_kernel():
    kc_row = _kc_row(
        "KC_CLU_DBS_001",
        "Core Point",
        path=["Clustering", "Density-Based Clustering (DBSCAN)", "Core Point"],
    )
    border = _candidate(
        "A border point is a point that is not a core point, but it falls within the neighborhood of a core point.",
        patch_heading="Core, border, and noise points",
        page_heading_norm="Core, border, and noise points",
        is_definition_like=True,
        relation_like=True,
    ...
    pack = compose_evidence_pack(
        kc_row=kc_row,
        step5_row=_step5_row(border, kc_id=kc_row["kc_id"], canonical_name=kc_row["canonical_name"]),
        sentence_context_index=None,
        cfg=_cfg(),
        source_manifests=_source_manifests(),
    )

    assert border["snippet"] not in [item["text"] for item in pack["slots"]["definition_kernel"]]
    assert "sibling_contrast" not in _ordered_roles(pack)


def test_noise_point_measurement_noise_context_does_not_become_positive_support():
    kc_row = _kc_row(
        "KC_CLU_DBS_003",
        "Noise Point",
        path=["Clustering", "Density-Based Clustering (DBSCAN)", "Noise Point"],
    )
    noisy_fragment = _candidate(
        "Notice that some of the noise points are intermixed with the non-noise points.",
        source_block_text=(
        ...
        patch_heading="Noise and Artifacts",
        page_heading_norm="Noise and Artifacts",
        preferred_support_role="definitional_anchor",
        anchor_quality="strong",
        has_context_completion_source=True,
        relation_like=False,
    ...
    pack = compose_evidence_pack(
        kc_row=kc_row,
        step5_row=_step5_row(noisy_fragment, kc_id=kc_row["kc_id"], canonical_name=kc_row["canonical_name"]),
        sentence_context_index=None,
        cfg=_cfg(),
        source_manifests=_source_manifests(),
    )

    assert noisy_fragment["snippet"] not in _positive_slot_texts(pack)
    assert pack["slots"]["context_completion"] == []
    assert pack["pack_quality"]["route"] in {"partial_grounded_packet", "insufficient_support_packet"}
...
def test_recurrent_global_candidate_without_target_binding_is_dropped():
    ...
    kc_rows = [
        _kc_row("KC_A", "Noise Point", path=["Clustering", "DBSCAN", "Noise Point"], parent_hier_node_id="topic-dbscan"),
        _kc_row(
            "KC_B",
            "Internal Indices Overview",
            path=["Clustering", "Evaluation", "Internal Indices Overview"],
            parent_hier_node_id="topic-clustering-evaluation",
        ),
        _kc_row(
            "KC_C",
            "k-Fold Cross Validation",
            path=["Evaluation", "Sampling", "k-Fold Cross Validation"],
            parent_hier_node_id="topic-sampling",
        ),
        _kc_row(
            "KC_D",
            "Model Selection Overview",
            path=["Classification", "Model Selection", "Model Selection Overview"],
            parent_hier_node_id="topic-model-selection",
        ),
    ]
    step5_rows_by_kc = {
        "KC_A": _step5_row(
        ...
        "KC_D": _step5_row(
            shared,
            _candidate(
                "Model selection is the task of choosing among competing models for a given data set.",
                block_id="doc-target:block:005",
                sent_idx=1,
                patch_heading="Model Selection",
                page_heading_norm="Model Selection",
                is_definition_like=True,
            ...
            kc_id="KC_D",
            canonical_name="Model Selection Overview",
        ),
    }
...
def test_regression_kfold_cross_validation_prefers_target_sentence():
    ...
    assert hyperparameter["snippet"] not in [item["text"] for item in pack["slots"]["formula_notation"]]
    assert target["snippet"] in _positive_slot_texts(pack)
...
def test_ordered_pack_for_drafting_excludes_sibling_contrast():
    kc_row = _kc_row(
        "KC_DE_PREP_003",
        "Duplicate Tuples",
        path=["Data Engineering", "Preparing the Data for Learning", "Duplicate Tuples"],
    )
    guardrail = _candidate(
        "Describe the potential problems with this algorithm if there are duplicate objects in the data set.",
        patch_heading="nearest neighbors",
        page_heading_norm="nearest neighbors",
        relation_like=False,
        competitor_token_hits=1,
        contamination_risk="medium",
    )
    target = _candidate(
        "Having duplicate tuples can be troublesome because they waste space and can be a source of inconsistency.",
        block_id="doc-target:block:002",
        sent_idx=1,
        patch_heading="Duplicate Tuples",
        page_heading_norm="Duplicate Tuples",
        preferred_support_role="explanatory_anchor",
        anchor_quality="usable",
        relation_like=True,
        exact_name_phrase=True,
    ...
    pack = compose_evidence_pack(
        kc_row=kc_row,
        step5_row=_step5_row(guardrail, target, kc_id=kc_row["kc_id"], canonical_name=kc_row["canonical_name"]),
        sentence_context_index=None,
        cfg=_cfg(),
        source_manifests=_source_manifests(),
    )
    ...
    assert "sibling_contrast" not in _ordered_roles(pack)
    assert guardrail["snippet"] not in [item["text"] for item in pack["ordered_pack_for_drafting"]]
    assert target["snippet"] in [item["text"] for item in pack["ordered_pack_for_drafting"]]
