...
def test_backend_policy_honors_definition_generation_mode_and_llm_budget():
    backend = build_drafting_backend(
        {
            "execution_mode": "llm",
            "provider": "ollama",
    ...
    assert policy.definition_generation_mode == "packet_multicandidate_v1"
    assert policy.max_llm_calls_per_kc == 11
...
def test_build_kc_draft_bundles_llm_packet_multicandidate_uses_support_pack_auditor_before_legacy(monkeypatch):
    kc_id = "KC_CLF_NB_003"
    canonical_name = "Prior Probability"
    rows = [
        _synthetic_support_pack_row(
            ...
            canonical_name=canonical_name,
            suffix="gloss",
            text="Prior Probability: the class-label distribution before attribute values are observed.",
        ),
        _synthetic_support_pack_row(
            kc_id=kc_id,
            canonical_name=canonical_name,
            suffix="mechanism",
            text="It captures the label distribution available before observing any attributes.",
        ),
    ]
    phase_log: list[str] = []
    ...
    monkeypatch.setattr(
        drafting_module,
        "_build_definition_packet_candidate_family_phase",
        lambda **kwargs: {
            "mode": "packet_multicandidate_v1",
            "candidate_count": 0,
            "candidates": [],
            "selected_candidate": {},
    ...
    def draft_invoker(request: dict[str, object]):
        ...
        if phase != "definition_support_pack_draft":
            raise AssertionError(f"unexpected draft phase: {phase}")
        roles = dict(dict(request["request_payload"]).get("support_pack_roles") or {})
        gloss_label = str((roles.get("definition_gloss") or [{}])[0].get("evidence_label") or "")
        mechanism_label = str((roles.get("mechanism_or_scope") or [{}])[0].get("evidence_label") or "")
        return {
            "definition": {
                "status": "grounded",
                "text": (
                    "Prior Probability is the class-label distribution before attribute values are observed, "
                    "and it directly predicts the final classifier output."
                ),
                "supporting_evidence_labels": [item for item in [gloss_label, mechanism_label] if item],
                "abstention_reason": "",
            }
        }, {"source": "test_definition_support_pack_draft"}

    def verify_invoker(request: dict[str, object]):
        phase = str(request["phase"])
        phase_log.append(phase)
        if phase != "definition_support_pack_audit":
            raise AssertionError(f"unexpected verify phase: {phase}")
        roles = dict(dict(request["request_payload"]).get("support_pack", {}).get("support_pack_roles") or {})
        gloss_label = str((roles.get("definition_gloss") or [{}])[0].get("evidence_label") or "")
        mechanism_label = str((roles.get("mechanism_or_scope") or [{}])[0].get("evidence_label") or "")
        return {
            "definition": {
                "status": "grounded",
                "text": "Prior Probability is the class-label distribution before attribute values are observed.",
                "supporting_evidence_labels": [item for item in [gloss_label, mechanism_label] if item],
                "abstention_reason": "",
            }
        }, {"source": "test_definition_support_pack_audit"}
    ...
    assert phase_log == ["definition_support_pack_draft", "definition_support_pack_audit"]
    assert bundle["definition_full_candidate"]["text"] == (
        "Prior Probability is the class-label distribution before attribute values are observed."
    )
    assert bundle["definition_full_candidate"]["selection_reason"] == (
        "definition_full_candidate_support_pack_audited"
    )
    assert llm_diag["effective_definition_phase"] == "support_pack"
    assert llm_diag["support_pack_mode_requested"] is True
    assert llm_diag["support_pack_audited_definition_selected"] is True
    assert llm_diag["packet_family_selected"] is False
    assert "accepted_support_pack_audited_definition" in bundle["selection_diagnostics"]["field_protection_actions"]
    assert stats["definition_packet_family_recoveries"] == 0
    assert stats["definition_support_pack_auditor_recoveries"] == 1
