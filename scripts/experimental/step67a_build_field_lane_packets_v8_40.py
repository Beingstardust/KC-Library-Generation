...
PACKETS_PATH = Path("data/processed/step67_sidecar_packets_40/2026-04-28_161958/evidence_packets.jsonl")
OVERLAY_PATH = Path("data/processed/kc_drafting_input_overlay/2026-04-28_002730/candidate_sentence_overlay.jsonl")
OUT_ROOT = Path("data/processed/step67_sidecar_lane_packets_v8_40")
...
def main() -> None:
    ...
    for packet in packets:
        ...
        output_rows.append({
            "lane_packet_contract_version": "step67a_field_lane_packets_v8_40",
            "run_id": run_id,
    ...
    summary = {
        "stage": "step67a_build_field_lane_packets_v8_40",
        "run_id": run_id,
    ...
    md: list[str] = []
    md.append("# Step 6.7A v8 40-KC diagnostic field-lane packet audit")
    md.append("")
    md.append("## Summary")
    md.append("")
    md.append("```json")
    ...
    print("STEP67A_V8_40_LANE_RUN_ID =", run_id)
    print("STEP67A_V8_40_LANE_DIR =", out_dir.as_posix())
    print("STEP67A_V8_40_LANE_PACKETS =", lane_packets_path.as_posix())
    print("STEP67A_V8_40_LANE_SUMMARY =", summary_path.as_posix())
    print("STEP67A_V8_40_LANE_AUDIT_MD =", audit_path.as_posix())
    ...
    print("STEP67A_V8_40_FIELD_LANE_PACKETS_DONE")
...
# V5 overrides
# Purpose:
# - keep V4/V4_40 conservative lane safety
# - demote advanced relation/equivalence evidence away from definition_lane
# - block broken direct-definition fragments
# - promote true "called/known as/table called X" definition clauses
# - avoid treating "used to/can be used/frequently used" as definitions
# ============================================================================

_ORIGINAL_CLASSIFY_ITEM_V4_40 = classify_item


def _item_combined_text_v5(item: dict[str, Any]) -> str:
    return "\n".join([
...
def _unsafe_fragment_ending_v5(text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return True
    ...
    last = ts[-1]
    unsafe_last = {
        "a", "an", "the", "of", "for", "to", "with", "without", "by", "in",
        "on", "from", "as", "and", "or", "but", "than", "into", "using",
        "greedy", "particular", "such",
    }

    if last in unsafe_last:
        # Some phrases such as "assigned to" are grammatical, so do not block
        # those cases blindly.
        safe_tail_phrases = {
            "assigned to",
            "according to",
        ...
        if not any(nt.endswith(p) for p in safe_tail_phrases):
            return True

    if re.search(r"\b(is|are|was|were|can|could|should|would|may|might|will)\s*$", nt):
        return True
...
def _advanced_relation_not_definition_v5(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    canonical = norm_text(profile.get("canonical_name") or "")

    relation_markers = [
        "special case of",
        "derived from a mixture model",
    ...
    if not any(marker in nt for marker in relation_markers):
        return False
    ...
    return not any(term in canonical for term in allowed_name_terms)


def _core_phrases_v5(profile: dict[str, Any]) -> list[str]:
    phrases = []
    ...
    for item in profile.get("phrases", []):
        if not isinstance(item, dict):
            continue
        source = str(item.get("source") or "")
        phrase = norm_text(item.get("phrase") or "")
        if source in {
            "canonical",
            "canonical_no_paren",
            "canonical_parenthetical",
...
def _safe_direct_definition_clause_v5(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return False

    # Explicitly avoid usage/scope clauses.
    usage_patterns = [
        r"\b(?:is|are|was|were)\s+(?:frequently\s+|commonly\s+|often\s+)?used\s+to\b",
        r"\b(?:is|are|was|were)\s+(?:frequently\s+|commonly\s+|often\s+)?used\s+for\b",
        r"\bcan\s+be\s+used\s+to\b",
        r"\bcan\s+be\s+used\s+for\b",
        r"\bis\s+applicable\s+to\b",
    ]
    if any(re.search(p, nt) for p in usage_patterns):
        return False

    phrases = _core_phrases_v5(profile)
    ...
    for phrase in phrases:
        ...
        # "SSE is an objective function ..."
        subject_definition = (
            pattern
            + r"(?:\s+\w+){0,6}\s+"
            + r"(?:is|are|refers|means)\b"
        )
        if re.search(subject_definition, nt):
            return True

        subject_defined = (
            pattern
            + r"(?:\s+\w+){0,6}\s+"
            + r"(?:is|are)\s+(?:defined|called|known|computed|calculated|measured)\b"
        )
        if re.search(subject_defined, nt):
            return True

        # Noun before target:
        # "a table called a confusion matrix"
        # "a measure known as ..."
        called_target = (
            r"\b(?:table|method|measure|metric|index|algorithm|procedure|quantity|coefficient|matrix|function|criterion)\b"
            + r"(?:\s+\w+){0,8}\s+"
            + r"(?:called|known\s+as|referred\s+to\s+as)\s+"
            + r"(?:a\s+|an\s+|the\s+)?"
            + pattern
        )
        if re.search(called_target, nt):
            return True

        # Definition verb before target:
        # "This approach is called three-fold cross-validation"
        before_target = (
            r"\b(?:called|known\s+as|defined\s+as|referred\s+to\s+as)\b"
            + r"(?:\s+\w+){0,8}\s+"
            + pattern
        )
        if re.search(before_target, nt):
            return True
...
def classify_item(item: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    result = _ORIGINAL_CLASSIFY_ITEM_V4_40(item, profile)

    combined_text = _item_combined_text_v5(item)
    lane = str(result.get("primary_lane") or "")

    # Hard demotion: true but overly narrow relation/equivalence evidence is not
    # a definition of the canonical KC.
    if lane == "definition_candidate" and _advanced_relation_not_definition_v5(profile, combined_text):
        return _set_lane(
            result,
            "context_candidate",
            "advanced relation/equivalence evidence is contextual, not a canonical definition",
        )

    # Hard demotion: broken fragments should not survive as definitions merely
    # because they contain "X is ...".
    if lane == "definition_candidate":
        quote_text = str(item.get("quote_text") or "")
        context_text = str(item.get("context_text") or "")

        if _unsafe_fragment_ending_v5(quote_text) and _unsafe_fragment_ending_v5(context_text):
            return _set_lane(
                result,
                "quarantine_or_irrelevant",
                "definition-like fragment is textually incomplete",
            )

    # Promotion: some true definition patterns were previously classified as
    # scope, especially "table called X" constructions.
    if lane in {"scope_candidate", "context_candidate", "quarantine_or_irrelevant"}:
        if (
            _safe_direct_definition_clause_v5(profile, combined_text)
            and not _advanced_relation_not_definition_v5(profile, combined_text)
            and not _core_point_sibling_fragment(profile, combined_text)
            and not _unsafe_fragment_ending_v5(combined_text)
        ):
            return _set_lane(
                result,
                "definition_candidate",
                "promoted safe direct definition clause",
            )
...
# V6 overrides
# Purpose:
# - keep V5's useful safe definition promotion, especially "table called X"
# - demote procedural, usage, relation, example, computation, and sibling evidence
#   away from definition_lane
# - promote narrow formula-definition clauses such as "X is given by equation"
# - preserve abstention when only scope/context evidence exists
# ============================================================================

_ORIGINAL_CLASSIFY_ITEM_V5_40 = classify_item


def _combined_text_v6(item: dict[str, Any]) -> str:
    return "\n".join([
        str(item.get("quote_text") or ""),
        str(item.get("context_text") or ""),
    ]).strip()
...
def _canonical_v6(profile: dict[str, Any]) -> str:
    return norm_text(profile.get("canonical_name") or "")


def _is_core_point_kc_v6(profile: dict[str, Any]) -> bool:
    return contains_phrase(_canonical_v6(profile), "core point")


def _is_kmeans_algorithm_kc_v6(profile: dict[str, Any]) -> bool:
    c = _canonical_v6(profile)
    return contains_phrase(c, "k means algorithm") or contains_phrase(c, "k-means algorithm")


def _is_gini_index_kc_v6(profile: dict[str, Any]) -> bool:
    return contains_phrase(_canonical_v6(profile), "gini index")


def _is_jaccard_kc_v6(profile: dict[str, Any]) -> bool:
    return contains_phrase(_canonical_v6(profile), "jaccard coefficient")


def _is_euclidean_distance_kc_v6(profile: dict[str, Any]) -> bool:
    return contains_phrase(_canonical_v6(profile), "euclidean distance")


def _is_hunts_algorithm_kc_v6(profile: dict[str, Any]) -> bool:
    c = _canonical_v6(profile)
    return contains_phrase(c, "hunt") and contains_phrase(c, "algorithm")


def _target_patterns_v6(profile: dict[str, Any]) -> list[str]:
    pats = []
    for phrase in _core_phrases_v5(profile):
        try:
            pats.append(_phrase_to_pattern(phrase))
        except Exception:
            pass
    return pats


def _target_mentioned_v6(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    return any(re.search(p, nt) for p in _target_patterns_v6(profile))


def _formula_definition_clause_v6(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt or not _target_mentioned_v6(profile, nt):
        return False

    formula_patterns = [
        r"\b(?:is|are)\s+given\s+by\s+(?:the\s+following\s+)?(?:equation|formula)\b",
        r"\b(?:is|are)\s+defined\s+by\s+(?:the\s+following\s+)?(?:equation|formula)\b",
        r"\b(?:is|are)\s+computed\s+as\b",
        r"\b(?:is|are)\s+calculated\s+as\b",
    ]

    if any(re.search(p, nt) for p in formula_patterns):
        return True

    return False


def _called_definition_clause_v6(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return False

    for pattern in _target_patterns_v6(profile):
        called_target = (
            r"\b(?:table|method|measure|metric|index|algorithm|procedure|quantity|coefficient|matrix|function|criterion)\b"
            + r"(?:\s+\w+){0,8}\s+"
            + r"(?:called|known\s+as|referred\s+to\s+as)\s+"
...
def _procedural_or_usage_not_definition_v6(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return False

    generic_nondefinition_patterns = [
        r"\b(?:is|are|was|were)\s+(?:then\s+)?(?:recursively\s+)?applied\b",
        r"\b(?:is|are|was|were)\s+(?:then\s+)?used\s+(?:to|for)\b",
        r"\bcan\s+be\s+used\s+(?:to|for)\b",
        r"\b(?:is|are|was|were)\s+(?:often\s+|frequently\s+|commonly\s+)?used\s+(?:to|for)\b",
        r"\b(?:is|are|was|were)\s+useful\s+when\b",
        r"\b(?:is|are|was|were)\s+applicable\s+to\b",
        r"\b(?:can|could|may)\s+be\s+extended\s+to\b",
        r"\b(?:can|could|may)\s+be\s+computed\b",
        r"\b(?:can|could|may)\s+be\s+calculated\b",
        r"\b(?:is|are|was|were)\s+computed\s+in\s+o\(",
        r"\b(?:is|are|was|were)\s+computed\s+at\s+each\b",
        r"\b(?:is|are|was|were)\s+updated\b",
        r"\battempts?\s+to\s+minimi[sz]e\b",
        r"\bdirectly\s+attempt\s+to\s+minimi[sz]e\b",
        r"\bstates?\s+that\b",
        r"\bchosen\s+instead\s+of\b",
        r"\bresults?\s+in\b",
        r"\bleads?\s+to\b",
    ]

    if any(re.search(p, nt) for p in generic_nondefinition_patterns):
        return True

    if _is_core_point_kc_v6(profile):
        if "border point" in nt or "noise point" in nt:
            return True
        if re.search(r"\bcore\s+points?\b.*\b(?:put|placed)\s+in\s+the\s+same\s+cluster\b", nt):
            return True

    if _is_hunts_algorithm_kc_v6(profile):
        if "recursively applied" in nt or "applied to each child" in nt:
            return True

    if _is_kmeans_algorithm_kc_v6(profile):
        if "steps 3 and 4" in nt or "minimize the sse" in nt or "objective function" in nt:
            return True

    if _is_gini_index_kc_v6(profile):
        if "candidate split" in nt or "split position" in nt or "updated gini" in nt or "computed in o" in nt:
            return True

    if _is_jaccard_kc_v6(profile):
        if "can be extended" in nt or "frequently used" in nt or "handle objects" in nt:
            return True

    if _is_euclidean_distance_kc_v6(profile):
        if "mahalanobis" in nt or "generalization of euclidean distance" in nt:
            return True
        if "case of" in nt and ("norm" in nt or "lp" in nt or "l p" in nt):
            return True
        if "relationship between" in nt:
            return True

    return False


def _definition_fragment_too_weak_v6(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return True

    # A definition should not be only a short procedural sentence, even if it has
    # the KC name and an "is" somewhere.
    if len(tokens(nt)) < 9 and not _called_definition_clause_v6(profile, nt) and not _formula_definition_clause_v6(profile, nt):
        return True

    if _unsafe_fragment_ending_v5(nt):
        return True
...
def classify_item(item: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    result = _ORIGINAL_CLASSIFY_ITEM_V5_40(item, profile)

    combined_text = _combined_text_v6(item)
    lane = str(result.get("primary_lane") or "")

    # First: demote bad definition evidence. This is the critical V6 correction.
    if lane == "definition_candidate":
        if _procedural_or_usage_not_definition_v6(profile, combined_text):
            return _set_lane(
                result,
                "context_candidate",
                "procedural/usage/relation evidence is contextual, not a canonical definition",
            )

        if _advanced_relation_not_definition_v5(profile, combined_text):
            return _set_lane(
                result,
                "context_candidate",
                "advanced relation/equivalence evidence is contextual, not a canonical definition",
        ...
        if (
            _definition_fragment_too_weak_v6(profile, quote_text)
            and _definition_fragment_too_weak_v6(profile, context_text)
            and not _called_definition_clause_v6(profile, combined_text)
            and not _formula_definition_clause_v6(profile, combined_text)
        ):
            return _set_lane(
                result,
                "quarantine_or_irrelevant",
                "definition-like fragment is too weak or incomplete",
            )

    # Second: narrowly promote true definitions that V4/V5 left in scope/context.
    if lane in {"scope_candidate", "context_candidate", "quarantine_or_irrelevant"}:
        if _procedural_or_usage_not_definition_v6(profile, combined_text):
            return result

        if _called_definition_clause_v6(profile, combined_text):
            return _set_lane(
                result,
                "definition_candidate",
                "promoted safe called/known-as definition clause",
            )

        if _formula_definition_clause_v6(profile, combined_text):
            return _set_lane(
                result,
                "definition_candidate",
                "promoted formula/equation definition clause",
            )
...
# V7 overrides
# Purpose:
# - keep V6's safety corrections
# - demote property/suitability/use-case claims from definition_lane
# - robustly promote target-mentioned formula/equation clauses
# ============================================================================

_ORIGINAL_CLASSIFY_ITEM_V6_40 = classify_item


def _formula_definition_clause_v7(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt or not _target_mentioned_v6(profile, nt):
        return False
    ...
    if "given by" in nt and ("equation" in nt or "formula" in nt):
        return True

    if "defined by" in nt and ("equation" in nt or "formula" in nt):
        return True

    if re.search(r"\b(?:is|are)\s+computed\s+as\b", nt):
        return True

    if re.search(r"\b(?:is|are)\s+calculated\s+as\b", nt):
        return True

    return False


def _property_or_suitability_not_definition_v7(profile: dict[str, Any], text: str) -> bool:
    nt = norm_text(text)
    if not nt:
        return False

    generic_patterns = [
        r"\bsatisf(?:y|ies|ied)\s+this\s+property\b",
        r"\bright\s+choice\s+of\b",
        r"\bright\s+choice\s+for\b",
        r"\bappropriate\s+choice\s+of\b",
        r"\bappropriate\s+choice\s+for\b",
        r"\bmost\s+commonly\s+used\b",
        r"\bcommonly\s+used\s+in\s+the\s+literature\b",
        r"\boften\s+used\b",
        r"\bfrequently\s+used\b",
        r"\buseful\s+when\b",
        r"\buseful\s+for\b",
        r"\bcan\s+avoid\b",
        r"\btendency\s+to\s+choose\b",
        r"\bhas\s+a\s+tendency\s+to\b",
        r"\bcan\s+define\b",
        r"\bcan\s+be\s+defined\b",
        r"\brecords\s+how\s+often\b",
        r"\bsummarizes?\s+the\s+performance\b",
    ]

    if any(re.search(p, nt) for p in generic_patterns):
        return True

    # Euclidean-specific relations are not definitions of Euclidean Distance.
    if _is_euclidean_distance_kc_v6(profile):
        euclidean_nondef = [
            "satisfies this property",
            "right choice of proximity measure",
            "case of a l",
            "case of an l",
            "lp norm",
            "l p norm",
            ...
            "relationship between",
            "metric is often used",
        ]
        if any(marker in nt for marker in euclidean_nondef):
            return True

    # Jaccard usage/extensions are not definitions unless formula/equation clause is present.
    if _is_jaccard_kc_v6(profile):
        if _formula_definition_clause_v7(profile, nt):
            return False
        jaccard_nondef = [
            "frequently used",
            "handle objects",
            "can be extended",
            "extended jaccard",
            "reduces to the jaccard coefficient",
        ]
        if any(marker in nt for marker in jaccard_nondef):
            return True
...
def classify_item(item: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    result = _ORIGINAL_CLASSIFY_ITEM_V6_40(item, profile)

    combined_text = _combined_text_v6(item)
    lane = str(result.get("primary_lane") or "")

    if lane == "definition_candidate":
        if _property_or_suitability_not_definition_v7(profile, combined_text):
            return _set_lane(
                result,
                "context_candidate",
                "property/suitability/use-case evidence is contextual, not a canonical definition",
            )
    ...
    if lane in {"scope_candidate", "context_candidate", "quarantine_or_irrelevant"}:
        ...
        if _formula_definition_clause_v7(profile, combined_text):
            return _set_lane(
                result,
                "definition_candidate",
                "promoted robust formula/equation definition clause",
            )
...
# Purpose:
# - keep V7 corrections
# - stop formula/equation promotion from leaking through long context_text
# - promote formula/equation definitions only when the local quote_text supports it
# - demote Jaccard relation/reduction text away from definition_lane
# ============================================================================

_ORIGINAL_CLASSIFY_ITEM_V7_40 = classify_item


def _quote_text_v8(item: dict[str, Any]) -> str:
    return str(item.get("quote_text") or "")


def _context_text_v8(item: dict[str, Any]) -> str:
    return str(item.get("context_text") or "")


def _quote_has_target_v8(profile: dict[str, Any], quote_text: str) -> bool:
    nt = norm_text(quote_text)
    if not nt:
        return False
    return any(re.search(p, nt) for p in _target_patterns_v6(profile))


def _quote_formula_definition_clause_v8(profile: dict[str, Any], quote_text: str) -> bool:
    nt = norm_text(quote_text)
    if not nt:
        return False

    if not _quote_has_target_v8(profile, nt):
        return False

    # Examples:
    # "The Jaccard coefficient, which is often symbolized by j, is given by the following equation:"
    # "The Gini index is defined by the following equation:"
    # "X is computed as ..."
    if "given by" in nt and ("equation" in nt or "formula" in nt):
        return True
...
def _jaccard_relation_not_definition_v8(profile: dict[str, Any], text: str) -> bool:
    if not _is_jaccard_kc_v6(profile):
        return False
    ...
    return any(marker in nt for marker in relation_markers)


def _formula_promotion_was_context_leak_v8(profile: dict[str, Any], item: dict[str, Any], result: dict[str, Any]) -> bool:
    reason = str(result.get("reason") or "")
    quote = _quote_text_v8(item)
    combined = _combined_text_v6(item)

    if "formula/equation" not in reason:
        return False

    # If quote itself does not have the formula clause, the promotion probably
    # came from surrounding context and is too unsafe for definition_lane.
    if not _quote_formula_definition_clause_v8(profile, quote):
        return True

    # Even if context contains formula text, a local Jaccard relation sentence
    # such as "extended Jaccard ... reduces to Jaccard" must not be definition.
    if _jaccard_relation_not_definition_v8(profile, quote):
        return True

    if _property_or_suitability_not_definition_v7(profile, quote):
        return True

    # If quote is safe, do not penalize merely because context has extra text.
    return False


def classify_item(item: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    result = _ORIGINAL_CLASSIFY_ITEM_V7_40(item, profile)
    ...
    if lane == "definition_candidate":
        if _formula_promotion_was_context_leak_v8(profile, item, result):
            return _set_lane(
                result,
                "context_candidate",
                "formula/equation promotion rejected because local quote is not a definition clause",
            )
        ...
        if _jaccard_relation_not_definition_v8(profile, quote):
            return _set_lane(
                result,
                "context_candidate",
                "Jaccard relation/reduction evidence is contextual, not a canonical definition",
            )
    ...
    # Quote-local formula promotion. This is intentionally stricter than V7.
    if lane in {"scope_candidate", "context_candidate", "quarantine_or_irrelevant"}:
        if _quote_formula_definition_clause_v8(profile, quote):
            if not _jaccard_relation_not_definition_v8(profile, quote):
                if not _property_or_suitability_not_definition_v7(profile, quote):
                    return _set_lane(
                        result,
                        "definition_candidate",
                        "promoted quote-local formula/equation definition clause",
                    )
