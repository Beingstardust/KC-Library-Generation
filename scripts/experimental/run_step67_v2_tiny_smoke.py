...
from kc_l.kc_drafting.status_integrity import apply_status_integrity_gate
...
def build_prompt(packet: Mapping[str, Any]) -> str:
    ...
    common = (
        ...
        "Write a COMPLETE account of this unit from the supplied passages. The packet carries every "
        "passage the source corpus offers about it, ranked by relevance, as coherent blocks rather "
        "than isolated sentences.\n"
        "Length must be governed by how much the passages actually support, not by a target length. "
        "Do not compress a well-supported unit into a single sentence, and do not pad a thin one.\n"
        ...
        "invent one.\n"
        "Each evidence item carries shape_tags (definition, formula, procedure, example) describing "
        "what KIND of content it holds. Use them to check your coverage: if procedure passages are "
        "present, your draft must state the procedure; if formula passages are present, your draft "
        "must carry the formula. The tags are information, not permission - draw on any passage that "
        "bears on the unit.\n"
        ...
        "Before finalizing, check every evidence_for_synthesis item shape_tags: if ANY item is tagged 'formula', your draft is INCOMPLETE unless it contains that formula with its symbols explained. If ANY item is tagged 'procedure', your draft is INCOMPLETE unless it states the steps. This is a hard requirement, not a suggestion - do not submit a draft that skips a formula or procedure the evidence actually provides.\n"
    ...
    if utype == "topic":
        ...
        return (
            ...
            + "Account for EVERY unit listed in direct_child_kcs: cover it if the passages support "
            + "it, or name it in child_kc_coverage_summary.missing_or_weak_child_kcs if they do not. "
            + "Do not silently omit a child unit.\n"
            ...
            + "Put the evidence ids you used in supporting_topic_evidence_ids and the child unit ids "
            + "they belong to in supporting_child_kc_ids.\n"
